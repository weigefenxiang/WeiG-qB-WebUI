...
async function prepareIndex(file){
  let html=await fs.readFile(file,'utf8');
  html=html.replaceAll('__WEIGG_GIT_SHA__',exactSha);
  if(!html.includes('data-weigg-virtual-sw-refresh')){
    if(!html.includes('</body>'))throw new Error(`Unable to install Virtual Pages worker refresh hook in ${file}`);
    html=html.replace('</body>',`${workerRefreshScript}</body>`);
  }
...
async function writeVersionedServiceWorker(){
  ...
  const sourceAnchor="const SOURCE_PRIVATE='./__source/private/';";
  const returnAnchor="  return new URL((kind==='public'?SOURCE_PUBLIC:SOURCE_PRIVATE)+safe,self.registration.scope).toString();";
  if(!source.includes(sourceAnchor)||!source.includes(returnAnchor))throw new Error('Virtual Pages Service Worker cache-bust anchors changed');
  source=source.replace(sourceAnchor,`const WEIGG_BUILD_SHA=${JSON.stringify(exactSha)};\n${sourceAnchor}`);
  source=source.replace(returnAnchor,"  const target=new URL((kind==='public'?SOURCE_PUBLIC:SOURCE_PRIVATE)+safe,self.registration.scope);\n  target.searchParams.set('v',WEIGG_BUILD_SHA);\n  return target.toString();");
...
const meta={branch,exactSha,productVersion,simulatorSha,builtAt:new Date().toISOString(),catalog:path.relative(projectRoot,catalogPath).replaceAll('\\','/'),productSource:'webui/** copied into __source with exact-SHA asset URLs and Virtual Pages worker refresh',webuiModified:false,pagesAdapted:true};
