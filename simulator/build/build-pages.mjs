...
const meta={branch,exactSha,productVersion,simulatorSha,builtAt:new Date().toISOString(),catalog:path.relative(projectRoot,catalogPath).replaceAll('\\','/'),localeOverlay:path.relative(projectRoot,localeOverlayPath).replaceAll('\\','/'),translatorBehavior:path.relative(projectRoot,translatorBehaviorPath).replaceAll('\\','/'),qbtEmulation:'validation-only/source-derived',settingsTranslationRouting:'single-compact-qb-settings-native-ir/native-minimal-QM/exact-bridge',productSource:branch==='dev'?'webui/** copied as the self-contained compact runtime; the full 65-release source catalog remains simulator-only evidence':'historical main preview materializes its legacy release-profile runtime from simulator-only source evidence',webuiModified:false,pagesAdapted:true};
