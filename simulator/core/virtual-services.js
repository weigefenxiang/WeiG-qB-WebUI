...
function cloneRssRule(rule){try{return JSON.parse(JSON.stringify(rule&&typeof rule==='object'?rule:{}));}catch{return{};}}
function rssRuleTorrentParams(rule){return rule&&rule.torrentParams&&typeof rule.torrentParams==='object'&&!Array.isArray(rule.torrentParams)?rule.torrentParams:{};}
function rssRuleValue(rule,legacyKey,modernKey){const params=rssRuleTorrentParams(rule);if(modernKey&&Object.prototype.hasOwnProperty.call(params,modernKey))return params[modernKey];return rule&&Object.prototype.hasOwnProperty.call(rule,legacyKey)?rule[legacyKey]:undefined;}
function rssRuleTags(rule){const value=rssRuleValue(rule,'tags','tags');return Array.isArray(value)?value.map(String):String(value||'').split(',').map(x=>x.trim()).filter(Boolean);}
export function rssSetRule(world,name,rule={}){
  const key=String(name||'').trim();if(!key)return false;
  const rules=rssRules(world),source=cloneRssRule(rule),previous=rules[key];
  if(!Object.prototype.hasOwnProperty.call(source,'enabled'))source.enabled=true;
  source.matchedIds=Array.isArray(source.matchedIds)?source.matchedIds:(Array.isArray(previous&&previous.matchedIds)?previous.matchedIds:[]);
  rules[key]=source;
  return true;
}
...
function applyRssRules(world,feed,article,now){
  ...
  for(const [name,rule] of Object.entries(rssRules(world))){
    ...
    addVirtualTorrent(world,{name:article.title,url:article.torrentURL,savepath:String(rssRuleValue(rule,'savePath','save_path')||''),category:String(rssRuleValue(rule,'assignedCategory','category')||''),tags:rssRuleTags(rule).join(',')},now);
