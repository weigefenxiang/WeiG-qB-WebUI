...
export function webseedCountForTorrent(world,hash){
  const torrent=torrentIndex(world).byHash.get(String(hash||''));
  if(!torrent||torrent.private)return 0;
  return hash32(`${world.seed}:${torrent.hash}:webseed-count`)%7;
}
export function indexedWebseedList(world,hash){
  const torrent=torrentIndex(world).byHash.get(String(hash||''));
  if(!torrent||torrent.private)return[];
  if(!Array.isArray(torrent.webseeds)){
    const count=webseedCountForTorrent(world,torrent.hash);
