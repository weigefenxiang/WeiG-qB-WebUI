...
function normalizeTranslation(value,numerus){
  if(numerus){
    if(!Array.isArray(value)||!value.length||value.some((item)=>!String(item||'').trim()))throw new Error('Numerus recovery vote requires non-empty plural forms.');
    return value.map((item)=>String(item));
  }
  if(Array.isArray(value))throw new Error('Scalar recovery vote cannot contain plural forms.');
  const text=String(value??'');
  if(!text)throw new Error('Recovery vote translation must not be empty.');
  return text;
}
function translationKey(value,numerus){return `${numerus?'1':'0'}\u0000${JSON.stringify(normalizeTranslation(value,numerus))}`;}
function messageKey(context,source){return `${String(context||'')}\u0000${String(source||'')}`;}
  ...
  for(const evidence of evidences){
    ...
    for(const localeEntry of evidence.locales){
      ...
      for(const message of localeEntry.messages||[]){
        const context=String(message?.context||''),source=String(message?.source||'');if(!context||!source)throw new Error(`${locale}: recovery message requires context + source.`);
...
export function buildQbNativeQmRecoveryEvidence(entries){
  ...
  for(const entry of entries){
    ...
    for(const message of parsed.messages){
      ...
      const msgIdentity=messageKey(message.context,message.source);let target=byMessage.get(msgIdentity);if(!target){target={context:String(message.context),source:String(message.source),candidates:new Map()};byMessage.set(msgIdentity,target);}
...
export function materializeQbNativeQmRecoveryUnion(evidence){
  ...
  return{schemaVersion:1,source:UNION_SOURCE,locales};
}
