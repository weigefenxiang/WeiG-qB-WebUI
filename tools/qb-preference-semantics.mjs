...
function inferGetter(expression, locals, declaredSessionGetters = new Map(), declaredPreferenceGetters = new Map(), declaredApplicationGetters = new Map(), declaredLocals = new Map(), declaredMembers = new Map(), declaredClassGetters = new Map(), singletonAliases = new Map()) {
  ...
  if (/\bQString::(?:number|from[A-Za-z_]\w*)\s*\(|\.toString\s*\(|\.join\s*\(|\bQStringLiteral\s*\(|\bQLatin1String\s*\(|\bQString\s*[({]/.test(value)) return {type: 'string', kind: 'STRING_EXPRESSION'};
  ...
  const arithmetic = value.match(/^([\s\S]+?)\s*([*/])\s*(-?\d+(?:\.\d+)?)$/);
  if (arithmetic) {
    const inner = inferGetter(arithmetic[1], locals, declaredSessionGetters, declaredPreferenceGetters, declaredApplicationGetters, declaredLocals, declaredMembers, declaredClassGetters, singletonAliases);
    if (inner.type === 'number') return {type: 'number', kind: 'NUMBER_ARITHMETIC'};
  }
