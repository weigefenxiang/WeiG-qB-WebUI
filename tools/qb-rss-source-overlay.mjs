...
import {assertRssSurfaceBindings,extractRssDownloaderSurface} from './qb-rss-surface-source.mjs';
...
export function enrichCatalogRssSurface(catalog,qbRoot){
  ...
  return catalog.map(profile=>{
    ...
    const source=rssDownloaderSource(qbRoot,tag);
    const rssDownloaderUi=assertRssSurfaceBindings(extractRssDownloaderSurface(source),`${qbVersion} RSS Downloader`);
    return{...profile,rssDownloaderUiSource:source?'qb-upstream-rss-downloader':'qb-upstream-native-surface-absent',rssDownloaderUi};
  });
}
