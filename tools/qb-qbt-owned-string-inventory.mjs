...
import {canonicalQbRef,extractQbtSourceRefs,qbSourceRefKey} from './qb-source-text.mjs';
...
export function extractQbtRefs(source,file=''){return extractQbtSourceRefs(source).map(ref=>({file,...ref}));}

export function collectSourceProvenQbRefs(catalog){
  const index=new Map();
  const remember=(profile,ref,{preferenceKey=null,uiKey=null,role=null}={})=>{
    const canonical=canonicalQbRef(ref);if(!canonical)return;
    const version=String(profile?.qbVersion||''),sourceSha=String(profile?.sourceSha||''),key=refKey(canonical.context,canonical.source);
    let item=index.get(key);
    if(!item){item={source:canonical.source,context:canonical.context,preferenceKeys:new Set(),uiKeys:new Set(),versions:new Set(),sourceShas:new Set(),roles:new Set()};index.set(key,item);}
