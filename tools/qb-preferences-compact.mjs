...
const CONTROL_FIELDS=['tab','sectionId','order','titleRef','descriptionRef','controlId','semantic','attributes','options','unitRef','staticDisabled','gates','projection'];
...
function compactControl(item,remember){if(!item)return null;const control=item.control||{},options=Array.isArray(control.options)&&control.options.length?control.options.map(option=>[option?.value??'',literalOrRef(option?.label,remember)]):0,gates=Array.isArray(item.dependencies?.gates)&&item.dependencies.gates.length?item.dependencies.gates.map(gate=>[String(gate?.controlId||''),String(gate?.preferenceKey||'')]):0,projection=item.projection&&typeof item.projection==='object'?stable(item.projection):0;return[String(item.tab||''),String(item.sectionId||''),Number(item.order)||0,remember(item.title),remember(item.description),String(control.id||''),String(control.semantic||''),control.attributes&&Object.keys(control.attributes).length?stable(control.attributes):0,options,remember(control.unit),control.staticDisabled===true?1:0,gates,projection];}
