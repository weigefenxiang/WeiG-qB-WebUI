...
const GRAPH_FAMILIES=['time-range'];
...
function compactGraphFamily(family,remember,rememberString){
  if(!family)return 0;
  const kind=GRAPH_FAMILIES.indexOf(String(family.kind||''));if(kind<0)return 0;
  if(family.kind==='time-range')return[kind,rememberString(family.display||'HH:mm'),compactGraphLabel(family.from?.label,remember),rememberString(family.from?.hourPreference),rememberString(family.from?.minutePreference),compactGraphLabel(family.to?.label,remember),rememberString(family.to?.hourPreference),rememberString(family.to?.minutePreference)];
  return 0;
}
function compactGraphTab(tab,remember,rememberString){
  ...
  const rows=(tab.rows||[]).map(item=>[rememberString(item?.id),rememberString(item?.parentFieldsetId),Number(item?.order)||0,GRAPH_TEMPLATES.indexOf(String(item?.template||'')),Number(item?.sourceOrder)||0,(item?.items||[]).map(child=>compactGraphItem(child,remember,rememberString)),compactGraphFamily(item?.family,remember,rememberString)]);
...
export function compileQbPreferencesCompact(sourceCatalog,canonicalCatalog=null){
  ...
  return{schemaVersion:2,source:'qb-upstream-preferences-native-surface-compact',catalogIdentity:identity,format:{release:['qbVersion','sourceSha'],ref:['context','source'],tab:['id','nativeId','titleRef'],section:['id','titleRef'],preference:CONTROL_FIELDS,descriptor:DESCRIPTOR_FIELDS,option:['value',['kind','refOrLiteral']],gate:['controlId','preferenceKey'],projection:{identity:['kind','safeWrite'],scale:['kind','safeWrite','scale'],'switch-map':['kind','safeWrite','values','defaultValue'],'compound-switch-map':['kind','safeWrite','values','defaultValue','auxiliary','writeCases'],'sentinel-gate':['kind','safeWrite','gateControlId','disabledValue','defaultValue','enabledWhen','scale?'],'presence-gate':['kind','safeWrite','gateControlId','disabledValue','enabledWhen'],unproven:['kind','safeWrite','readFactor?','writeFactor?']},graph:{string:'1-based graphStrings index, 0=null',roles:GRAPH_ROLES,semantics:GRAPH_SEMANTICS,templates:GRAPH_TEMPLATES,contentKinds:GRAPH_CONTENT_KINDS,families:GRAPH_FAMILIES,predicates:GRAPH_PREDICATES,actions:GRAPH_ACTIONS,tab:['fieldsets','rows'],fieldset:['id','parentId','title','template','sourceOrder','legendControls'],row:['id','parentFieldsetId','order','template','sourceOrder','items','family'],control:['kind=0','id','preferenceKey','role','semantic','staticDisabled','label','adornment','suffix','condition','dynamicOptions','writeOnly','displayFormat'],helper:['kind=1','id','label','action','condition'],content:['kind=2','contentKind','id','forControlId','label','items']}},releases:identities.map(item=>[item.qbVersion,item.sourceSha]),refs,graphStrings,tabs,sections,preferences,descriptors,graphs};
}
...
export function expandQbPreferencesCompact(compact,qbVersion){
  ...
  const graphFamily=(raw)=>{if(!Array.isArray(raw)||raw.length===0)return null;const kind=GRAPH_FAMILIES[raw[0]]||'';if(kind==='time-range')return{kind,display:graphString(raw[1])||'HH:mm',from:{label:graphLabel(raw[2]),hourPreference:graphString(raw[3]),minutePreference:graphString(raw[4])},to:{label:graphLabel(raw[5]),hourPreference:graphString(raw[6]),minutePreference:graphString(raw[7])}};return null;};
  const graphTab=(raw)=>{if(!Array.isArray(raw))return null;return{fieldsets:(raw[0]||[]).map(row=>({id:graphString(row[0]),parentId:graphString(row[1]),title:graphLabel(row[2]),template:GRAPH_TEMPLATES[row[3]]||'',sourceOrder:row[4]||0,legendControls:(row[5]||[]).map(graphControl)})),rows:(raw[1]||[]).map(row=>({id:graphString(row[0]),parentFieldsetId:graphString(row[1]),order:row[2],template:GRAPH_TEMPLATES[row[3]]||'',sourceOrder:row[4]||0,items:(row[5]||[]).map(graphItem),...(graphFamily(row[6])?{family:graphFamily(row[6])}:{})}))};};
