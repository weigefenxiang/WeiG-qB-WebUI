...
import {canonicalQbHtmlText,parseQbtSourceRef,qbSourceRefKey} from './qb-source-text.mjs';
...
function sourceRefIdentity(ref){
  if(!ref)return'';
  if(ref.source&&ref.context)return qbSourceRefKey(ref.context,ref.source);
...
function buildSourceOwnershipIndex(markup,tabs,fieldsets,caches){
  ...
  for(const control of caches.controls.values()){
    ...
    if(!candidates.length&&row){
      const rowLabels=labels.filter(label=>inside(row,label.start)),rowControls=[...caches.controls.values()].filter(item=>inside(row,item.start));
      const unlabeledControls=rowControls.filter(item=>!explicitIds.has(item.id)&&!String(attrText(item.sourceAttrs||'','aria-labelledby')||'').trim());
      const labelledCheckboxes=rowLabels.filter(label=>{
        if(!label.forId)return false;
        const gate=caches.controls.get(String(label.forId));
        return gate?.semantic==='checkbox'&&rowControls.some(item=>item.id===gate.id);
      });
      if(unlabeledControls.length===1&&unlabeledControls[0].id===control.id&&labelledCheckboxes.length===1&&labelledCheckboxes[0].end<=control.start)add(labelledCheckboxes[0],2,'E3:compound-row-gate');
    }
    ...
    if(!candidates.length&&nearestField){
      ...
      const tabRows=rows.filter(item=>inside(tab.range,item.start)),unrowedFieldControls=directFieldControls.filter(item=>!nearestContaining(tabRows,item.start));
      const evidence=controlLegend?'E3:legend-control':(control.tag==='table'?'E3:structured-group':(!row&&unrowedFieldControls.length===1?'E3:standalone-fieldset-group':(directFieldControls.length===1?'E3:single-control-group':'')));
...
function sourceDisplayBinding(markup,controlId){
  const text=String(markup||''),escaped=escapeRegex(controlId),pref='([A-Za-z_$][\\w$]*)',formatter='([A-Za-z_$][\\w$]*)';
  const modern=new RegExp('document\\.getElementById\\(\\s*["\\\']'+escaped+'["\\\']\\s*\\)\\s*\\.\\s*value\\s*=\\s*'+formatter+'\\s*\\(\\s*pref\\s*\\.\\s*'+pref+'\\s*\\)').exec(text);
  const shorthand=new RegExp('\\$\\(\\s*["\\\']'+escaped+'["\\\']\\s*\\)\\s*\\.\\s*value\\s*=\\s*'+formatter+'\\s*\\(\\s*pref\\s*\\.\\s*'+pref+'\\s*\\)').exec(text);
  const legacy=new RegExp('\\$\\(\\s*["\\\']'+escaped+'["\\\']\\s*\\)\\s*\\.(?:setProperty|set)\\(\\s*["\\\']value["\\\']\\s*,\\s*'+formatter+'\\s*\\(\\s*pref\\s*\\.\\s*'+pref+'\\s*\\)\\s*\\)').exec(text);
  const hit=modern||shorthand||legacy;
  return hit?{formatter:hit[1],preferenceKey:hit[2],dialect:modern?'modern-value':(shorthand?'shorthand-value':'legacy-setter')}:null;
}
function sourceDisplayFormat(markup,controlId,binding){
  const text=String(markup||''),assignment=binding||sourceDisplayBinding(markup,controlId);
  if(!assignment)return null;
  const name=escapeRegex(assignment.formatter);
  const arrow=new RegExp('(?:const|let|var)\\s+'+name+'\\s*=\\s*\\([^)]*\\)\\s*=>\\s*\\{([\\s\\S]{0,700}?)\\n\\s*\\};').exec(text);
  const expression=new RegExp('(?:const|let|var)\\s+'+name+'\\s*=\\s*function\\s*\\([^)]*\\)\\s*\\{([\\s\\S]{0,700}?)\\n\\s*\\};').exec(text);
  const declared=new RegExp('function\\s+'+name+'\\s*\\([^)]*\\)\\s*\\{([\\s\\S]{0,700}?)\\n\\s*\\}').exec(text);
  const body=String((arrow&&arrow[1])||(expression&&expression[1])||(declared&&declared[1])||'');
...
function sourcePresentedPreferenceKey(markup,control,binding,displayFormat){
  if(!control||!binding||displayFormat!=='zero-pad-2'||!binding.preferenceKey)return null;
...
function sameGraphLabel(a,b){
  if(!a||!b)return false;
  if(Object.prototype.hasOwnProperty.call(a,'source')||Object.prototype.hasOwnProperty.call(b,'source'))return String(a.source||'')===String(b.source||'')&&String(a.context||'')===String(b.context||'');
  if(Object.prototype.hasOwnProperty.call(a,'literal')||Object.prototype.hasOwnProperty.call(b,'literal'))return String(a.literal||'')===String(b.literal||'');
  return false;
}
function sourceRowFamily(template,items){
  if(template!=='inline-multi-control'||!Array.isArray(items)||items.length!==4)return null;
  const controls=items.filter(item=>item?.kind==='control'&&item.preferenceKey);
  if(controls.length!==4||!controls.every(item=>item.displayFormat==='zero-pad-2'))return null;
  const suffix=controls.map(item=>String(item?.suffix?.literal||''));
  if(suffix[0]!==':'||suffix[1]!==''||suffix[2]!==':'||suffix[3]!=='')return null;
  const from=controls[0].label||controls[1].label,to=controls[2].label||controls[3].label;
  if(!from||!to||sameGraphLabel(from,to))return null;
  if(controls[0].label&&controls[1].label&&!sameGraphLabel(controls[0].label,controls[1].label))return null;
...
function buildControlGraph(markup,tabs,fieldsets,caches,preferences,writeOnlyByControl){
  ...
  for(const tab of tabs){
    ...
    for(const row of tabRows){
      ...
      rows.push({id:tab.id+':row:'+rows.length,parentFieldsetId:parent?fieldIds.get(parent):null,order:rows.length,sourceOrder:0,_sourcePos:row.start,template,items,...(family?{family}:{})});
    }
    for(const control of caches.controls.values()){
