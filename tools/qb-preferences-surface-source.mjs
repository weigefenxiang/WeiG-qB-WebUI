...
export function extractQbPreferencesNativeSurface({preferencesSource='',toolbarSource='',preferenceDescriptors=[]}={}){
  const descriptors=new Map((preferenceDescriptors||[]).map(item=>[String(item?.key||''),item]));
  const keys=[...descriptors.keys()].filter(Boolean),ui=extractQbPreferenceUiFacts(preferencesSource,keys),supplement=extractQbPreferencesCompositeUiFacts(preferencesSource,keys);
