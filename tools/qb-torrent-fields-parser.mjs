...
function extractBlockFromOpen(text,open,label){let depth=0,inString=false,quote='',escape=false;for(let i=open;i<text.length;i++){const ch=text[i];if(inString){if(escape){escape=false;continue;}if(ch==='\\'){escape=true;continue;}if(ch===quote)inString=false;continue;}if(ch==='"'||ch==="'"||ch==='`'){inString=true;quote=ch;continue;}if(ch==='{')depth++;else if(ch==='}'&&--depth===0)return text.slice(open+1,i);}throw new Error(`${label}: unterminated block`);}
function jsString(value){return String(value||'').replace(/\\(['"\\])/g,'$1').replace(/\\n/g,'\n').replace(/\\t/g,'\t');}
function qbtRef(value){const match=String(value||'').match(/QBT_TR\(([\s\S]*?)\)QBT_TR\[CONTEXT=([^\]]+)\]/);if(!match)return null;return{source:match[1].trim(),context:String(match[2]||'').trim()};}
function torrentsClassBody(source,context){const text=String(source||'');let match=text.match(/\bclass\s+TorrentsTable\s+extends\s+DynamicTable\s*\{/);if(match){const open=text.indexOf('{',match.index);return extractBlockFromOpen(text,open,`${context}: class TorrentsTable`);}match=text.match(/\b(?:const|let|var)\s+TorrentsTable\s*=\s*new\s+Class\s*\(\s*\{/);if(match){const open=text.indexOf('{',match.index);return extractBlockFromOpen(text,open,`${context}: TorrentsTable Class`);}throw new Error(`${context}: unable to locate TorrentsTable implementation`);}
function initColumnsBody(classBody,context){let match=/\binitColumns\s*\(\s*\)\s*\{/.exec(classBody);if(!match)match=/\binitColumns\s*:\s*function\s*\(\s*\)\s*\{/.exec(classBody);if(!match)throw new Error(`${context}: TorrentsTable.initColumns missing`);const open=classBody.indexOf('{',match.index+match[0].length-1);return extractBlockFromOpen(classBody,open,`${context}: TorrentsTable.initColumns`);}
...
export function extractTorrentTableColumns(dynamicTableSource,context='qB source'){
  const classBody=torrentsClassBody(dynamicTableSource,context),body=initColumnsBody(classBody,context),columns=[];
  ...
  if(!columns.length)throw new Error(`${context}: unable to extract TorrentsTable native columns`);
  ...
  if(new Set(columns.map(item=>item.key)).size!==columns.length)throw new Error(`${context}: duplicate TorrentsTable native column key`);
  return columns;
}
