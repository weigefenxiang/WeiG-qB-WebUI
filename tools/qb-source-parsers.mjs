...
function inferredSetterHint(key, segment, source) {
...
function supplementCompoundSetterHints(body, hints) {
  const directMap = new RegExp(String.raw`m\s*\[\s*(${KEY_LITERAL_SOURCE})\s*\]\s*\.\s*to(?:Bool|Int|UInt|LongLong|ULongLong|Double|Float|String|ByteArray|Array|List|Map|Hash|Object)\s*\(`, 'g');
  for (const match of String(body || '').matchAll(directMap)) {
    const key = parseKeyLiteral(match[1]);
    const hint = inferredSetterHint(key, match[0], 'UPSTREAM_SETTER_MAP_ACCESS');
    const previous = hints.get(key);
    if (hint && (!previous || !previous.writeType)) hints.set(key, hint);
  }

  const source = String(body || '');
  const iterators = [];
  const findIter = new RegExp(String.raw`([A-Za-z_$][\w$]*)\s*=\s*m\s*\.\s*(?:constFind|find)\s*\(\s*(${KEY_LITERAL_SOURCE})\s*\)`, 'g');
  for (const match of source.matchAll(findIter)) {
    const key = parseKeyLiteral(match[2]);
    if (key) iterators.push({ name: match[1], key, position: match.index ?? 0 });
  }
  for (let index = 0; index < iterators.length; index++) {
    const iterator = iterators[index];
    const nextSame = iterators.slice(index + 1).find(item => item.name === iterator.name);
    const segment = source.slice(iterator.position, nextSame ? nextSame.position : source.length);
    const escaped = iterator.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const use = new RegExp('\\b' + escaped + '\\s*\\.\\s*value\\s*\\(\\s*\\)\\s*\\.\\s*to(?:Bool|Int|UInt|LongLong|ULongLong|Double|Float|String|ByteArray|Array|List|Map|Hash|Object)\\s*\\(', 'g');
    const match = use.exec(segment);
    const hint = match && inferredSetterHint(iterator.key, match[0], 'UPSTREAM_SETTER_ITERATOR');
    const previous = hints.get(iterator.key);
    if (hint && (!previous || !previous.writeType)) hints.set(iterator.key, hint);
  }
  return hints;
}
...
export function extractPreferenceSetterHints(source, label = 'source') {
  ...
  supplementCompoundSetterHints(body, hints);
  return hints;
}
