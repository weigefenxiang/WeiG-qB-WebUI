...
function runtime(catalog){
  ...
  const sources={};for(const name of ['release-profile.js','capabilities.js','torrent-semantics.js'])sources[name]=fs.readFileSync(path.join(root,'webui/private/scripts',name),'utf8');
  ...
  for(const name of ['release-profile.js','capabilities.js','torrent-semantics.js'])vm.runInNewContext(sources[name],context,{filename:name});
...
export async function summarizeProductProfile(profile,catalog,shared=null){
  const rt=shared||runtime(catalog),R=rt.W.ReleaseProfile,C=rt.W.CapabilityRegistry,T=rt.W.TorrentSemantics;
