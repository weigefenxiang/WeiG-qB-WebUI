...
### SETTINGS-PROJECTION — full source graph intersected with runtime truth
The visible qB Options projection is `exact source tabs / fieldsets / rows / controls / auxiliary gates / actions / copy nodes / order ∩ current GET app/preferences`. Options, adornments, local labels, note/help/list/hint copy, dependencies and raw/UI projections come from the exact admitted `qbVersion + sourceSha` graph. Preferences absent from the current daemon response are not rendered. When exact source proves compound composition or an auxiliary gate, presentation preserves that semantic grouping through generic templates; it must not flatten it into an invented per-Preference model or recreate it with key-specific code.

### SETTINGS-WRITE-PROVENANCE — every write fails closed
A Preference may enter `app/setPreferences` only when all of these are true at the moment of the write: certified exact/equivalent release identity; source-proven `setPreferences` action; current `app/preferences` contains the key; exact descriptor proves setter/writable/type agreement; source `projection.safeWrite === true`; every runtime-relevant source dependency predicate is resolved and satisfied; and the outgoing raw value matches the proven write type. Save-time `writableDraft()` repeats the same gate as defense in depth. An unknown predicate is a proof gap, not an editable false value and not evidence that source fidelity is complete. Historical switch-map/read-only or otherwise unproven inverses never invent a write.
...
### SETTINGS-COPY — exact qB copy closure covers every displayed graph ref
Preference copy and every displayed source-owned Control Graph ref reuse `W.I18n`'s existing exact-release qB-owned copy registry. The dependency closure includes tabs, fieldsets, controls, options, helpers/actions, local labels, adornments and note/help/list/hint copy. If the exact release's target locale has an official translation, the compact native/QM or proven bridge path must include it; an omitted bundle ref must not silently cause mixed-language fallback. When the official target translation genuinely does not exist, fallback is only the upstream English source. Settings must not create a second qB translation database; broader cross-domain copy ownership remains C-phase work.
...
### SETTINGS-VISUAL — generic packing, one draft and canonical primitives
Settings reuses existing `settings-section`, `settings-grid`, `setting-row`, `field-input setting-input`, `switch-control`, shared Dialog and `W.Components.selectControl()` primitives. Desktop may pack compatible short source rows/clusters into the same visual line using generic graph/geometry rules; textarea, structured/table, note/list or long-copy rows remain full width. Multiline controls may expose an expanded canonical Dialog, but inline and expanded editors mutate the same Settings draft and use the same final Save/reread owner. Feature-local skins, key-specific layout maps, a second editor state or Mobile-only business state are prohibited.
...
### SETTINGS-FIDELITY-CENSUS — no false-green structural completeness
Source/admission accounting must independently cover source controls, actions, adornments/options/local labels, non-control copy nodes and runtime-relevant behavior predicates. A release is not full-fidelity complete while any required source node is unaccounted, ownership is duplicated, or a runtime-relevant predicate remains unknown. Explicit reviewed exclusions are allowed only when they preserve fail-closed product behavior and are visible in validation evidence.
