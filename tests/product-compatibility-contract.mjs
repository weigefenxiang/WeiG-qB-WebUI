...
assert(T.matchesStatus({state:'checkingUP',progress:1,dlspeed:0,upspeed:0},'checking',[]));assert.deepEqual(visual({state:'pausedUP',progress:1,upspeed:0}),{state:'complete-idle',active:false,percent:100});assert.deepEqual(visual({state:'pausedDL',progress:.4,dlspeed:0}),{state:'paused',active:false,percent:40});assert.deepEqual(visual({state:'stalledUP',progress:1,upspeed:0}),{state:'seed-idle',active:false,percent:100});assert.deepEqual(visual({state:'uploading',progress:1,upspeed:240}),{state:'seed',active:true,percent:100});assert.deepEqual(visual({state:'uploading',progress:1,upspeed:0}),{state:'seed',active:true,percent:100});
...
assert(T.isSupportedFilter('private'));assert.deepEqual(visual({state:'stoppedUP',progress:1,upspeed:0}),{state:'complete-idle',active:false,percent:100});assert.deepEqual(visual({state:'stoppedDL',progress:.4,dlspeed:0}),{state:'paused',active:false,percent:40});assert.deepEqual(visual({state:'stalledUP',progress:1,upspeed:0}),{state:'seed-idle',active:false,percent:100});assert.deepEqual(visual({state:'downloading',progress:.4,dlspeed:0}),{state:'download',active:true,percent:40});
