...
const releaseSource=String(await fs.readFile(new URL('../tools/qb-release-torrent-surface.mjs',import.meta.url)));
assert.ok(releaseSource.includes("readOptionalSource('src/webui/www/private/views/transferlist.html')"));
assert.ok(releaseSource.includes("readOptionalSource('src/webui/www/private/scripts/mocha-init.js')"));
assert.ok(releaseSource.includes('torrentMenuHandlerSource=[transferListSource,mochaInitSource,clientSource]'),'official qB Torrent menu helper implementations must be traced across transferlist + mocha-init + client source');
...
const capabilities=String(await fs.readFile(new URL('../webui/private/scripts/capabilities.js',import.meta.url)));
const selection=String(await fs.readFile(new URL('../webui/private/scripts/selection.js',import.meta.url)));
