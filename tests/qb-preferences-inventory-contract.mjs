...
assert.deepEqual(inventory.preferenceRefs.map(item=>item.key),['locale','future_gate','future_limit'],'independent raw preference inventory must support dot and bracket pref syntax');
...
assert.throws(()=>assertCompleteSourceCensus(missed.preferences,'Preferences preference inventory'),/unaccounted=future_unmapped/,'future source syntax or controls must fail closed instead of silently lowering a mapping ratio');
...
console.log('qB Preferences independent inventory contract passed: raw tabs, pref references, and native controls are censused independently; unknown upstream additions fail closed until mapped or explicitly reviewed.');
