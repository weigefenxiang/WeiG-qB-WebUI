...
try{
  for(const name of ['legacy','modern']){
    ...
    assert(await page.locator('#transfer-capsule').count()===1,`${name}: unified Transfer capsule missing`);await page.locator('.transfer-runtime-capsule__stats').click();await page.waitForSelector('#transfer-stats-dialog[open]');assert(await page.locator('#transfer-chart').count()===1,`${name}: realtime Transfer chart missing`);await page.locator('#transfer-chart-window .ui-select__trigger').click();const windows=await page.locator('#weigg-floating-layer .ui-select__menu:not([hidden]) .ui-select__option').allTextContents();assert(windows.join('|').includes('1 min')&&windows.join('|').includes('5 min')&&windows.join('|').includes('15 min'),`${name}: Transfer chart windows incomplete`);await page.keyboard.press('Escape');
