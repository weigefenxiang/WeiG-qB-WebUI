...
assert(design.includes('Semantic Ownership 3.7'),'DESIGN authority must identify current semantic ownership');
for(const token of ['COMPAT-DEGRADE-001','PERF-COMPAT-001','PROGRESSIVE-DATA-001','TIMESERIES-001','INTERACTION-TEST-001','CONTROL-SKIN-001','DIALOG-SCROLL-001','NATIVE-THEME-001','OWNER-RETIRE-001','CURRENT-ONLY-001','FILE-NAMING-001','TEST-CURRENT-001','CAPABILITY-OWNER-001','CAPABILITY-RANGE-001','CAPABILITY-COST-001','RENDERED-SIGNAL-001','HEADER-END-ANCHOR-001','STATUS-EXPLAIN-001'])assert(design.includes(token),`DESIGN hard rule missing ${token}`);
for(const token of ['ROUTE-OWNER-001','RATE-UNIT-001','LAYOUT-OWNER-001','EXACT-HEAD-001','CONTROL-SKIN-001','DIALOG-SCROLL-001','NATIVE-THEME-001','OWNER-RETIRE-001','CAPABILITY-OWNER-001'])assert(archDocs.includes(token),`Architecture docs missing ${token}`);
for(const token of ['SELECT-TOPLAYER-001','SELECT-SCROLL-001','INTERACTION-TEST-001','PROGRESSIVE-DATA-001','CONTROL-SKIN-001','DIALOG-SCROLL-001','NATIVE-THEME-001','HEADER-SEARCH-001','HEADER-END-ANCHOR-001','RENDERED-SIGNAL-001'])assert(uiDocs.includes(token),`UI/cache contract missing ${token}`);
for(const token of ['EXACT-HEAD-001','qB4 Private click','12 h','ALT Apply','CONTROL-SKIN-001','DIALOG-SCROLL-001','NATIVE-THEME-001','CAPABILITY-RANGE-001','STATUS-EXPLAIN-001'])assert(implementationDocs.includes(token),`Current implementation notes missing ${token}`);
...
console.log('Runtime contract passed: current-only assets, one detected qB client, range-driven capabilities, explicit transfer ingest, one Connection owner/signal, end-anchored header, bounded transfer and docs authority.');
