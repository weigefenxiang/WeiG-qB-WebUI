...
void AppController::preferencesAction()
{
    ...
    data["legacy_hour"] = 8;
    data["legacy_min"] = 30;
    data[u"modern_hour"_s] = 20;
    data[u"modern_min"_s] = 45;
    data[u"modern2_hour"_s] = 21;
    data[u"modern2_min"_s] = 15;
    data[QLatin1String("legacy_iter_bool")] = true;
    data[QLatin1String("legacy_iter_path")] = QStringLiteral("/webui");
}
void AppController::setPreferencesAction()
{
    if (m.contains("legacy_hour") && m.contains("legacy_min"))
        pref->setLegacyTime(QTime(m["legacy_hour"].toInt(), m["legacy_min"].toInt()));
    if (const auto hourIter = m.constFind(u"modern_hour"_s), minIter = m.constFind(u"modern_min"_s)
        ; (hourIter != m.constEnd()) && (minIter != m.constEnd()))
        pref->setModernTime({hourIter.value().toInt(), minIter.value().toInt()});
    if (const auto hourIter = m.constFind(u"modern2_hour"_s), minIter = m.constFind(u"modern2_min"_s)
        ; (hourIter != m.constEnd()) && (minIter != m.constEnd()))
        pref->setModernTime2({hourIter.value().toInt(), minIter.value().toInt()});
    QVariantMap::const_iterator it;
    if ((it = m.find(QLatin1String("legacy_iter_bool"))) != m.constEnd())
        pref->setLegacyIteratorBool(it.value().toBool());
    if ((it = m.find(QLatin1String("legacy_iter_path"))) != m.constEnd())
        pref->setLegacyIteratorPath(it.value().toString());
}
`;
const compoundSetters=extractPreferenceSetterHints(compoundSource,'compound pair source');
for(const key of ['legacy_hour','legacy_min','modern_hour','modern_min','modern2_hour','modern2_min','legacy_iter_bool','legacy_iter_path']){
  const expectedType=key==='legacy_iter_bool'?'boolean':key==='legacy_iter_path'?'string':'number';
  assert.equal(compoundSetters.get(key)?.writeType,expectedType,key+' must inherit exact setter evidence from compound/legacy iterator map access');
  assert.equal(compoundSetters.get(key)?.setterConfidence,'HIGH');
}
const compoundDescriptors=Object.fromEntries(extractPreferenceDescriptors(compoundSource,'compound pair source').map(item=>[item.key,item]));
for(const key of ['legacy_hour','legacy_min','modern_hour','modern_min','modern2_hour','modern2_min','legacy_iter_bool','legacy_iter_path']){
  assert.equal(compoundDescriptors[key]?.typeAgreement,'EXACT',key+' read/write types must agree');
  assert.equal(compoundDescriptors[key]?.writable,true,key+' compound setter must not be misclassified read-only');
}
...
console.log('qB preference source parser contract passed: qB4 m.find + qB5 constFind/direct getter-setter truth are separated, enum fallbacks stay non-default provenance, conflicts fail closed, and write-only fields never escape app/preferences.');
