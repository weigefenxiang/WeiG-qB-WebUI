...
function expectedFilterMode(profile,filter){const value=canonicalFilter(filter);if(value==='private')return(profile?.torrentInfoParameters||[]).includes('private')?'local':'unavailable';if(canonicalFilters(profile).includes(value))return'native';if(canDeriveFilter(profile,value))return'local';return'unavailable';}
function expectedRenderedFilters(profile){const out=canonicalFilters(profile);for(const name of derivedOrder)if(canDeriveFilter(profile,name)&&!out.includes(name))out.push(name);if((profile?.torrentInfoParameters||[]).includes('private')&&!out.includes('private'))out.push('private');return out;}
...
try{
  ...
  let state=await inspect(page);assert.equal(state.certified,true,'qB 4.1.0 must bind an exact certified source profile');assert.deepEqual(state.filters,expectedRenderedFilters(catalog[0]),`qB 4.1.0 rendered filters must equal native + locally derivable source truth: ${JSON.stringify(state)}`);assert.deepEqual(state.filterModes,expectedModes(catalog[0]),'qB 4.1.0 filter modes must distinguish native from locally derived filters');assert.equal(state.tag,false,'qB 4.1.0 unsupported Tags facet must be hidden');assert.equal(state.tagsSupported,false,'qB 4.1.0 must not claim Tags support');assert.equal(state.privateSupported,false,'qB 4.1.0 must not claim Private filter support');assert.equal(state.stalledSupported,(catalog[0].torrentFilters||[]).includes('stalled'),'qB 4.1.0 native Stalled capability truth must remain source-derived');
