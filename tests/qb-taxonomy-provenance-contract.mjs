...
const sourceActionDescriptor=action=>{if(!profile)return undefined;if(profile.fallback===true)return null;return descriptor(action);};
const WeiG={
...
console.log(`QBClient taxonomy provenance passed: ${calls.length} allowed HTTP calls; direct and sync category reads normalize source-proven historical shapes through CapabilityRegistry, writes remain independently guarded, and fallback profiles fail closed.`);