...
assert(workflow.includes('git -c core.quotePath=false diff --name-only -z "$PUBLISHED_SHA" "$GITHUB_SHA"'),
  'Pages source relay must scan the entire published-payload-to-current-dev range, not only the latest push diff');
...
console.log('Pages source drain contract passed: a docs-only follow-up cannot strand an older materialized dev payload behind unpublished Pages-relevant changes.');
