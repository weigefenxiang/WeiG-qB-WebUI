...
assert(progressCss.includes('.mobile-card-meta--rail{display:grid!important;grid-template-columns:minmax(0,1fr)!important;grid-template-rows:auto auto!important')&&progressCss.includes('.mobile-card-progress{width:100%;max-width:none;min-width:0;margin-left:0;grid-template-columns:minmax(0,1fr) max-content')&&!uiCss.includes('.mobile-card-meta--rail{display:grid!important'),'Canonical Mobile progress stylesheet must own stacked metrics/progress geometry without a competing UiSystem grid override');
