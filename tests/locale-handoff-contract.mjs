...
assert.equal(legacySpecialLocales.length,50,'qB 4.1.9.1 must retain its frozen 50-locale source-backed inventory');
...
{
  const runtime=await makeRuntime(['zh-CN'],{locale:'zh_CN',alternative_webui_enabled:true},{sourceLocales:legacySpecialLocales,qbVersion:'4.1.9.1',compatQbVersion:'4.1.9',resolutionMode:'EQUIVALENT',localeHtml:localeHtmlFor(['zh_CN'])});
  const values=runtime.window.WeiG.I18n.localeOptions().map(item=>item.value),evidence=runtime.window.WeiG.I18n.localeInventoryEvidence();
  assert.ok(legacySpecialLocales.every(value=>values.includes(value)),'4.1.9.1 partial runtime probe must not replace the frozen 50-locale source baseline');
  assert.ok(values.length>=50,'4.1.9.1 language UI must never collapse to a single current locale');
  assert.equal(evidence.classification,'PARTIAL_OR_INVALID');
  assert.equal(evidence.selectedOwner,'source-baseline');
  assert.equal(evidence.sourceCount,50);
  assert.equal(evidence.runtimeCount,1);
  assert.ok(evidence.missing.length>0);
}
for(const qbVersion of ['4.6.7','5.2.0']){
  const profile=lkg.profiles.find(item=>item.qbVersion===qbVersion);assert.ok(profile,`${qbVersion} must remain an exact admitted locale profile`);const sourceLocales=lkg.localeSets[profile.localeSet];assert.ok(sourceLocales&&sourceLocales.length>1,`${qbVersion} source locale inventory must be complete`);
  const runtime=await makeRuntime(['zh-CN'],{locale:'zh_CN',alternative_webui_enabled:true},{sourceLocales,qbVersion,localeHtml:localeHtmlFor(['zh_CN'])});
  const options=runtime.window.WeiG.I18n.settingOptions('locale'),evidence=runtime.window.WeiG.I18n.localeInventoryEvidence();
  assert.ok(Array.isArray(options)&&options.length===sourceLocales.length,`${qbVersion} Settings locale owner must keep the full native source inventory when the runtime probe is current-only`);
  assert.deepEqual(Array.from(options,item=>item.value),sourceLocales,`${qbVersion} Behavior and WeiG language controls must consume the exact same ordered qB locale inventory`);
  assert.equal(evidence.classification,'PARTIAL_OR_INVALID');assert.equal(evidence.selectedOwner,'source-baseline');assert.equal(evidence.runtimeCount,1);
}
{
  const qbVersion='4.6.7',profile=lkg.profiles.find(item=>item.qbVersion===qbVersion),sourceLocales=lkg.localeSets[profile.localeSet],lateSource=[];
  const runtime=await makeRuntime(['zh-CN'],{locale:'zh_CN',alternative_webui_enabled:true},{sourceLocales:lateSource,qbVersion,localeHtml:localeHtmlFor(['zh_CN'])});
  const earlyState=runtime.window.WeiG.I18n.settingOptionsState('locale'),earlyOptions=runtime.window.WeiG.I18n.settingOptions('locale');
  assert.equal(earlyState.ready,false,'an early current-only locale probe must remain WAITING_SOURCE instead of being accepted as a complete Settings provider');
  assert.equal(Array.from(earlyOptions||[]).length,1,'an early current-only probe may expose one current/fallback value but must not masquerade as the final supported inventory');
  lateSource.push(...sourceLocales);
  await runtime.window.WeiG.I18n.loadLocaleOptions();
  const readyState=runtime.window.WeiG.I18n.settingOptionsState('locale'),readyOptions=runtime.window.WeiG.I18n.settingOptions('locale');
  assert.equal(readyState.ready,true,'the same locale provider must transition to READY when exact qB source inventory becomes available');
  assert.ok(readyState.revision>earlyState.revision,'source-ready reconciliation must advance the provider revision so an already-open Settings surface can rerender');
  assert.deepEqual(Array.from(readyOptions,item=>item.value),sourceLocales,'qB 4.6.7 stale current-only cache must be invalidated and replaced by the exact source-native language inventory');
}
...
assert.ok(settingsSource.includes("global.addEventListener('weig:setting-optionschange'")&&settingsSource.includes("e.detail.key!=='locale'"),'an already-open Settings route must reconcile in place when the canonical locale options revision changes.');
{
  ...
  assert.equal(evidence.selectedOwner,'source-baseline');
  assert.equal(runtime.window.WeiG.I18n.localeOptions().length,50);
}
{
  const runtime=await makeRuntime(['zh-CN'],{locale:'zh',alternative_webui_enabled:true},{sourceLocales:legacySpecialLocales,qbVersion:'4.1.9.1',compatQbVersion:'4.1.9',resolutionMode:'EQUIVALENT',localeHtml:localeHtmlFor(legacySpecialLocales)});
  const evidence=runtime.window.WeiG.I18n.localeInventoryEvidence();
  assert.equal(evidence.classification,'COMPLETE_NATIVE_SET');
  assert.equal(evidence.selectedOwner,'runtime-complete');
...
}
{
  ...
  await ready;
  assert.ok(Date.now()-started<2200,'locale navigation handoff must be bounded so a refused/missing pagehide can never freeze app readiness forever');
}
{
...
assert.ok(sessionSource.includes('function installLocaleReadyBootstrap()')&&sessionSource.includes('var original=I.ready')&&sessionSource.includes('function waitForLocaleNavigation()')&&sessionSource.includes("global.addEventListener('pagehide',finish,{once:true})")&&sessionSource.includes('setTimeout(finish,1500)')&&sessionSource.includes('I.applyLocale(result.prefs.locale)'),'Session must gate existing app W.I18n.ready through a bounded navigation handoff after qB verifies the browser-selected locale');
assert.ok(!sessionSource.includes('return new Promise(function(){})'),'Session must never use an unbounded never-resolving Promise to hand continuation to reload');
