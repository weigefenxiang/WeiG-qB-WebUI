...
if(docs){assert(/4\.1\.0/.test(docs)&&/official stable/i.test(docs),'compatibility docs must state the qB 4.1.0 official-stable support floor');assert(/未来[^\n]*(?:stable|major)|future[^\n]*(?:stable|major)/i.test(docs),'compatibility docs must prioritize future stable/new-major compatibility');assert(/4\.0\.x[\s\S]{0,500}(?:不是当前|未来可选|future optional)/i.test(docs),'compatibility docs must keep qB4.0.x outside the current implementation/CI scope');assert(/source-derived|源码生成|上游源码/i.test(docs),'compatibility docs must describe source-derived release truth');assert(/Linux.*Chrome|Windows.*Chrome|Upstream Compatibility Audit|全 stable/i.test(docs),'compatibility docs must distinguish browser evidence from full upstream stable audit');assert(!/55[^\n]*(?:stable|稳定)[^\n]*40[^\n]*4\.x[^\n]*15[^\n]*5\.x/i.test(docs),'compatibility docs must not hard-code obsolete stable counts');}
