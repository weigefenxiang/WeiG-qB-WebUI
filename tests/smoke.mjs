...
const required = [
  ...
  'webui/private/scripts/i18n.js','webui/private/scripts/i18n-v030.js','webui/private/scripts/i18n-v034.js','webui/private/scripts/i18n-v036.js','webui/private/scripts/core.js','webui/private/scripts/semantics-v036.js','webui/private/scripts/settings-schema.js','webui/private/scripts/settings-translations.js','webui/private/scripts/qb-client.js','webui/private/scripts/components.js','webui/private/scripts/app.js','webui/private/scripts/settings-v034.js','webui/private/scripts/ux-v021.js','webui/private/scripts/spatial-v022.js','webui/private/scripts/v030.js','webui/private/scripts/v036.js','webui/private/scripts/adaptive-v036.js','webui/private/scripts/advanced-v036.js','webui/private/scripts/logs-v032.js',
  'tests/compat-v030.mjs','tests/log-compat-v032.mjs','tests/browser-logs-v033.mjs','tests/settings-v034.mjs','tests/browser-settings-v034.mjs','tests/cache-contract-v035.mjs','tests/ui-contract-v036.mjs','tests/mobile-contract-v036.mjs','tests/advanced-contract-v036.mjs','tests/semantics-contract-v036.mjs','tests/browser-ui-v036.mjs','tests/browser-mobile-v036.mjs','tests/compat-matrix-v036.mjs','tests/fixtures/qb-compat-matrix.json','tests/live-v036.sh',
...
for(const token of ['css/app.css?v=__WEIGG_GIT_SHA__','css/logs-v032.css?v=__WEIGG_GIT_SHA__','css/mobile-v036.css?v=__WEIGG_GIT_SHA__','scripts/logs-v032.js?v=__WEIGG_GIT_SHA__','scripts/i18n-v036.js?v=__WEIGG_GIT_SHA__','scripts/semantics-v036.js?v=__WEIGG_GIT_SHA__','scripts/adaptive-v036.js?v=__WEIGG_GIT_SHA__','scripts/i18n-v034.js?v=__WEIGG_GIT_SHA__','scripts/settings-v034.js?v=__WEIGG_GIT_SHA__','no-store, no-cache, must-revalidate','name="weigg-build-sha" content="__WEIGG_GIT_SHA__"']) if(!index.includes(token)) throw new Error(`Runtime/cache asset invariant missing: ${token}`);
...
for(const token of ["'v036.logs.follow':'Follow latest'","'v036.logs.follow':'跟随最新'","'v036.settings.timeZone':'Display time zone'","'v036.storage.free':'Free'","'v036.storage.free':'可用'","'v036.mobile.count':'{count} Torrents'","'v036.facet.path':'路径'",'W.V036I18n']) if(!i18n036.includes(token)) throw new Error(`v0.3.6 translation overlay missing: ${token}`);
...
for(const token of ['TorrentSemantics','isPrivate','isPt','isPrivateOrPt','trackerHost','normalizeRules']) if(!semantics.includes(token)) throw new Error(`Torrent semantic token missing: ${token}`);
const app=fs.readFileSync('webui/private/scripts/app.js','utf8');
for(const token of ['pageSize','buildCatalog','renderTrackerNav','renderSettings','openColumns','TorrentSemantics','isPrivateOrPt','VirtualList','normalizeTracker']) if(!app.includes(token)) throw new Error(`App architecture token missing: ${token}`);
...
for(const token of ['#torrent-list{flex:1 1 0','mobile-card-meta','status-storage','stalled-up','#search-view>.tool-page','.mobile-command-bar','--mobile-stat-label-size','data-primary-scroll','is-data-focus']) if(!mobile036.includes(token)) throw new Error(`v0.3.6 mobile adaptive style missing: ${token}`);
...
for(const token of ['compactMetricText','free_space_on_disk','getMainData(storageRid)','30000','advanced-v036.js','pageContracts','primaryScrollOwners','mobile-command-bar','syncFacetSummaries','registerDataViewport','setDataViewportFocus']) if(!adaptive036.includes(token)) throw new Error(`v0.3.6 adaptive runtime missing: ${token}`);
