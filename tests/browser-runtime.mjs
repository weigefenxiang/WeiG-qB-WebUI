...
async function selectTab(page,tab){await page.locator(`#settings-tabs [data-settings-tab="${tab}"]`).click();await page.waitForFunction(t=>document.querySelector('#settings-tabs [data-settings-tab="'+t+'"]')?.classList.contains('is-active'),tab);await page.waitForSelector('#settings-content .setting-row:not([hidden])');}
...
try{
  for(const name of ['legacy','modern']){
    ...
    for(const tab of ['downloads','connection','speed','bittorrent','webui','advanced']){
      ...
      assert(await twoColumns(page),`${name}/${tab}: SettingsGrid is not two columns at 1366px`);
      assert(await alignment(page),`${name}/${tab}: SettingRow copy/control alignment failed`);
      assert(await noLegacySettings(page),`${name}/${tab}: legacy Settings DOM owner reappeared`);
    }
    for(const key of ['save_path','proxy_ip','proxy_username','web_ui_address','web_ui_username','alternative_webui_path']){
      const row=page.locator(`[data-setting-key="${key}"]`);if(await row.count())assert((await row.getAttribute('data-setting-span'))!=='full',`${name}: ${key} must not force full span`);
    }
    await selectTab(page,'about');assert(await page.locator('.brand-identity .brand__mark').count()===1,`${name}: About shared BrandMark missing`);assert(await page.locator('.about-surface .setting-row').count()===0,`${name}: About must use FactRow, not SettingRow`);
    assert(await page.locator('.brand-cluster>.brand-mark-home').count()===1&&await page.locator('.brand-cluster>.brand-name-home').count()===1,`${name}: Header brand targets not separated`);
    assert(await page.locator('#github-link').count()===1&&await page.locator('#blog-link').count()===1&&await page.locator('#logout-btn').count()===1,`${name}: Header utilities missing`);
    await page.setViewportSize({width:1000,height:768});await selectTab(page,'downloads');await page.waitForTimeout(60);assert(!(await twoColumns(page)),`${name}: SettingsGrid did not collapse at narrow width`);assert(await alignment(page),`${name}: narrow SettingRow alignment failed`);
  ...
  }
  {
    ...
    const context=await browser.newContext({viewport:{width:1366,height:768}}),page=await context.newPage();await page.goto(`http://${host}:${port}/modern/#/`,{waitUntil:'networkidle'});await page.waitForSelector('#logout-btn');
    await openSettings(page);await selectTab(page,'connection');await page.evaluate(()=>{location.hash='#/';});await page.waitForFunction(()=>location.hash==='#/');
    await page.locator('#logout-btn').click();await page.waitForSelector('#login-form',{timeout:4000});assert(variants.modern.logout===1,'Verified logout did not call auth/logout exactly once');assert(variants.modern.probeAfterLogout>=1,'Verified logout did not probe a protected endpoint after logout');
    await page.goBack({waitUntil:'domcontentloaded'}).catch(()=>null);await page.waitForSelector('#login-form',{timeout:4000});assert(await page.locator('#app').count()===0,'Browser Back restored the private application shell after logout');await context.close();
  }
  {
  ...
  console.log('Semantic runtime browser regression passed for all Settings tabs, qB 4.1.9.1 + 5.2.0, verified logout, Back/BFCache guard and auth bypass.');
