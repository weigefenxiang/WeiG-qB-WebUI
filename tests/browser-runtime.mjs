...
import {settingsTranslationData} from '../tools/qb-webui-catalog.mjs';
...
if(exactProfiles.some(profile=>!profile))throw new Error('Browser runtime fixture requires frozen exact qB 4.1.9.1 and 5.2.0 release profiles.');
const settingsTranslations=settingsTranslationData(exactProfiles);
...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,`http://${host}:${port}`),m=url.pathname.match(/^\/(legacy|modern|bypass)(?:\/(.*))?$/);if(!m){res.writeHead(404);return res.end('not found');}const v=variants[m[1]],rel=m[2]||'';if(rel.startsWith('api/v2/'))return api(req,res,v,rel.slice(7),url);if(rel==='data/qb-releases.json')return json(res,exactProfiles);if(rel==='data/qb-settings-translations.json')return json(res,settingsTranslations);if(rel==='weigg-install.json')return json(res,{version:productVersion,gitSha:'fixture-sha',qbPath:'/config/weigg-qb-webui',hostPath:'/srv/qb/config/weigg-qb-webui'});const root=v.session?privateRoot:publicRoot,requested=rel||'index.html',file=path.resolve(root,requested);if(!(file===root||file.startsWith(root+path.sep))){res.writeHead(403);return res.end('forbidden');}const body=await fs.readFile(file);res.writeHead(200,{'content-type':mime[path.extname(file).toLowerCase()]||'application/octet-stream','cache-control':'no-store'});res.end(body);}catch(e){res.writeHead(e?.code==='ENOENT'?404:500,{'content-type':'text/plain; charset=utf-8'});res.end(String(e));}});
...
try{
  for(const name of ['legacy','modern']){
    ...
    assert(await page.locator('#settings-tabs [data-settings-tab="speed"]').count()===1,`${name}: canonical Speed settings tab is missing`);
    for(const tab of ['downloads','connection','speed','bittorrent','webui']){await selectTab(page,tab);assert(await twoColumns(page),`${name}/${tab}: SettingsGrid is not two columns at 1366px`);assert(await alignment(page),`${name}/${tab}: SettingRow copy/control alignment failed`);assert(await noLegacySettings(page),`${name}/${tab}: legacy Settings DOM owner reappeared`);}
    await selectTab(page,'speed');assert(await page.locator('[data-setting-key="dl_limit"],[data-setting-key="up_limit"],[data-setting-key="alt_dl_limit"],[data-setting-key="alt_up_limit"]').count()===4,`${name}: canonical Speed rate-limit preferences are missing`);
    await selectTab(page,'webui');assert(await page.locator('[data-setting-key="web_ui_http3_enabled"] input[type="checkbox"]').count()===1,`${name}: future Web UI boolean preference was not auto-routed/rendered`);
    await selectTab(page,'advanced');const structured=page.locator('[data-setting-key="future_structured_policy"] textarea[readonly]');assert(await structured.count()===1,`${name}: unknown structured preference is not visible/read-only`);assert((await structured.inputValue()).includes('"mode": "auto"'),`${name}: structured preference lost JSON value`);
    for(const key of ['save_path','proxy_ip','proxy_username','web_ui_address','web_ui_username','alternative_webui_path']){for(const tab of ['downloads','connection','webui']){await selectTab(page,tab);const row=page.locator(`[data-setting-key="${key}"]`);if(await row.count())assert((await row.getAttribute('data-setting-span'))!=='full',`${name}: ${key} must not force full span`);}}
  ...
  console.log('Semantic runtime browser regression passed: six qB Settings surfaces including canonical Speed + future preference fallback + deep link, qB 4.1.9.1/5.2.0, verified logout, Back/BFCache guard and auth bypass.');
