...
async function selectTab(page,tab){await page.locator(`#settings-tabs [data-settings-tab="${tab}"]`).click();await page.waitForFunction(t=>document.querySelector('#settings-tabs [data-settings-tab="'+t+'"]')?.classList.contains('is-active'),tab);}
...
async function alignment(page){return page.evaluate(()=>[...document.querySelectorAll('#settings-content .setting-row:not([hidden])')].slice(0,8).every(r=>{const a=r.getBoundingClientRect(),copy=r.querySelector('.setting-copy')?.getBoundingClientRect(),ctl=r.querySelector('.setting-control-slot')?.getBoundingClientRect();return !!copy&&!!ctl&&copy.left>=a.left-1&&copy.left<ctl.left&&Math.abs(a.right-ctl.right)<=18;}));}
...
try{
  for(const name of ['legacy','modern']){
    ...
    await page.goto(`http://${host}:${port}/${name}/#/`,{waitUntil:'networkidle'});await page.waitForSelector('#torrent-list');await openSettings(page);await selectTab(page,'downloads');await page.waitForSelector('#settings-content .setting-row');
    assert(await twoColumns(page),`${name}: SettingsGrid is not two columns at 1366px`);assert(await alignment(page),`${name}: SettingRow left/right alignment failed`);
    await selectTab(page,'webui');const span=await page.locator('[data-setting-key="alternative_webui_path"]').getAttribute('data-setting-span');assert(span!=='full',`${name}: ordinary Alternative WebUI path must not force full span`);
    ...
    await page.setViewportSize({width:1000,height:768});await selectTab(page,'downloads');await page.waitForTimeout(60);assert(!(await twoColumns(page)),`${name}: SettingsGrid did not collapse at narrow width`);
    assert(errors.length===0,`${name}: browser errors: ${errors.join(' | ')}`);await context.close();
  }
  {
    const context=await browser.newContext({viewport:{width:1366,height:768}}),page=await context.newPage();await page.goto(`http://${host}:${port}/modern/#/`,{waitUntil:'networkidle'});await page.waitForSelector('#logout-btn');await page.locator('#logout-btn').click();await page.waitForSelector('#login-form',{timeout:4000});assert(variants.modern.logout===1,'Verified logout did not call auth/logout exactly once');assert(variants.modern.probeAfterLogout>=1,'Verified logout did not probe a protected endpoint after logout');await context.close();
  }
  {
  ...
  console.log('Semantic runtime browser regression passed for qB 4.1.9.1, qB 5.2.0, responsive Settings, shared BrandMark and verified logout.');
