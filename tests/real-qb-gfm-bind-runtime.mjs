...
export function bindRuntimeEvidence(runtime,{root=repoRoot,mode,version,coreMode,searchMode}={}){
  ...
  const smokePass=exactStableIdentity(runtime.runtime_version,version)&&runtime.cleanup_result==='PASS';
  return {
    ...runtime,
    schemaVersion:4,
    gfm_mode:mode,
...
if(invoked){
  ...
  const bound=bindRuntimeEvidence(runtime,{mode:a.mode,version:a.version,coreMode:a['core-mode'],searchMode:a['search-mode']});
  const tmp=`${file}.tmp-${process.pid}`;
  fs.writeFileSync(tmp,`${JSON.stringify(bound,null,2)}\n`);
  fs.renameSync(tmp,file);
  console.log(`Bound G-FM ${a.mode} runtime evidence for qB ${a.version}: smoke=${bound.runtime_smoke_result}, core=${bound.core_mode}, search=${bound.search_mode}`);
}
