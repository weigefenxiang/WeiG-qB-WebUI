...
console.log(`CI contract passed for WeiG ${version}: exact qB 4.1.0+ source catalog is audited once on dev, executed through every formal product compatibility owner, compact-packed for qB WebUI static delivery, embedded in the reusable candidate artifact, and that exact artifact is promoted/released without a main rebuild; Pages consumes hash-bound Frozen LKG + Locale facts, dispatches on the real dev/main ref so deploy-pages build identity is exact, main promotion reuses only the validated dev site artifact, and hosted Chrome remains canonical.`);
