...
assert(catalogPack.includes('10*1024*1024')&&catalogPack.includes('Packed qB release catalog')&&catalogPack.includes('runtimeCatalogData')&&catalogPack.includes('SETTINGS_FIELDS')&&catalogPack.includes('settingsTranslationPath')&&catalogPack.includes("path.join(path.dirname(outputPath),'qb-settings')")&&catalogPack.includes('assertStaticSize(`qB Settings translation shard'),'catalog packaging must preserve runtime compatibility semantics, strip all-release Settings translation payloads, shard exact official copy, and fail closed at qB WebUI static-file limits');
