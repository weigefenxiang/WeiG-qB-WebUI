...
assert(driver.includes('DEFAULT_FETCH_TIMEOUT_MS=15000')&&driver.includes('AbortController')&&driver.includes('context.addInitScript'),'browser-driver must bound both Node and in-browser fetches so live Pages gates cannot hang indefinitely');
...
assert(catalogPack.includes('10*1024*1024')&&catalogPack.includes('PROJECT_MAX_SINGLE_FILE_BYTES=5*1024*1024')&&catalogPack.includes('Packed qB release catalog')&&catalogPack.includes('runtimeCatalogData')&&catalogPack.includes('SETTINGS_FIELDS')&&catalogPack.includes('settingsNativeLocales')&&catalogPack.includes('settingsTranslationLocales')&&catalogPack.includes('settingsTranslationPath')&&catalogPack.includes("fs.writeFileSync(path.join(dataDir,'qb-settings-native.txt')")&&catalogPack.includes("path.join(dataDir,'..','..','translations')")&&catalogPack.includes('renderLocaleQm(messages)')&&catalogPack.includes("path.join(dataDir,'qb-settings')")&&catalogPack.includes('assertStaticSize(`qB Settings compatibility shard'),'catalog packaging must preserve runtime compatibility semantics, strip all-release Settings translation bodies, emit native QBT_TR/minimal QM assets, retain only exact gap compatibility shards, and fail closed at qB/project static-file limits');
for(const [name,section] of [['release_fixture',releaseFixture],['release_catalog_base',releaseBase],['release_locale_enrich',releaseEnrich],['release_catalog_merge',releaseMerge],['release_upstream_audit',releaseAudit],['release_lkg',releaseLkg],['release_product_matrix',releaseProduct],['release_compatibility',releaseCompat],['browser',linux],['windows_browser',windows],['release_candidate',candidate]]){
...
const pages=read('.github/workflows/pages.yml'),pagesBuild=jobSection(pages,'build','deploy'),pagesDeploy=jobSection(pages,'deploy','verify'),pagesVerify=jobSection(pages,'verify','reuse_main'),pagesReuse=jobSection(pages,'reuse_main','promote_deploy'),pagesPromote=jobSection(pages,'promote_deploy'),pagesSource=read('.github/workflows/pages-source.yml');
...
assert(!/\n\s+paths:/.test(pagesSource),'Pages source relay must materialize every exact dev/main HEAD; path filters can leave the public dev installer payload behind branch HEAD');
...
assert(!pagesBuild.includes('Drain legacy default-branch Pages watcher')&&!pagesBuild.includes('event=workflow_run'),'retired legacy Pages watcher drain must stay removed after default-branch trigger migration');
assert(pagesDeploy.includes('Refuse stale dev Pages deployment')&&pagesDeploy.includes('/commits/dev')&&pagesDeploy.includes('REMOTE_SHA=')&&pagesDeploy.includes('Refusing stale dev Pages deployment'),'dev Pages deploy must re-check current dev HEAD immediately before deploy-pages and fail closed if stale');
...
assert(/timeout-minutes:\s*8/.test(pagesVerify)&&/fail-fast:\s*false/.test(pagesVerify)&&/max-parallel:\s*16/.test(pagesVerify),'Pages live verification must use bounded 16-way independent browser jobs');
for(const name of ['core','locale-bootstrap','release-profile','services','mobile-layout','preferences-anchor','preferences-0','preferences-1','preferences-2','preferences-3','preferences-4','preferences-5','preferences-6','preferences-7','preferences-8','preferences-9'])assert(pagesVerify.includes(`name: ${name}`),`Pages verify matrix missing ${name}`);
assert(pagesVerify.includes('tests/pages-live-locale-bootstrap.mjs')&&pagesVerify.includes('tests/pages-live-acceptance.mjs')&&pagesVerify.includes('tests/pages-live-preferences.mjs')&&pagesVerify.includes('tests/pages-live-release-profile.mjs')&&pagesVerify.includes('tests/pages-live-services.mjs')&&pagesVerify.includes('tests/pages-live-mobile-layout.mjs'),'Pages verify matrix must cover locale bootstrap, core, all stable Preferences, release-profile, services and mobile layout');
assert(pagesVerify.includes('pref_mode: anchor')&&pagesVerify.includes('pref_mode: shard')&&pagesVerify.includes("pref_total: '10'")&&pagesVerify.includes("pref_index: '9'"),'Pages Preferences live audit must split all stable profiles across one anchor plus ten shards');
assert(pagesVerify.includes("WEIGG_BROWSER_FETCH_TIMEOUT_MS: '15000'")&&pagesVerify.includes("WEIGG_PAGES_PROFILE_TIMEOUT_MS: '45000'")&&pagesVerify.includes('timeout --signal=TERM --kill-after=10s 7m node "$WEIGG_VERIFY_SCRIPT"'),'Pages verification must fail bounded at request, profile and process/job layers');
const localeLive=read('tests/pages-live-locale-bootstrap.mjs'),labLive=read('simulator/lab/lab.js');
assert(localeLive.includes("locale:'zh-CN'")&&localeLive.includes("prefs.json?.locale,'zh_CN'")&&localeLive.includes('const second=await launchLabSession(2)'),'Pages locale gate must prove two fresh Lab sims in one zh-CN browser both persist qB zh_CN');
assert(labLive.includes("'weigg.localeBootstrap.v2'")&&labLive.includes('resetLabLocaleBootstrap()'),'Lab launcher must clear simulator-only locale bootstrap state before allocating each fresh sim');
...
assert(pagesPromote.includes('Refuse stale main Pages deployment')&&pagesPromote.includes('/commits/main')&&pagesPromote.includes('REMOTE_SHA=')&&pagesPromote.includes('Refusing stale main Pages deployment'),'main Pages deploy must re-check current main HEAD immediately before deploy-pages and fail closed if stale');
...
console.log(`CI contract passed for WeiG ${version}: exact qB 4.1.0+ source catalog is audited once on dev, executed through every formal product compatibility owner, runtime-projected into a translation-body-free compatibility catalog with native QBT_TR/minimal QM ownership plus exact gap-only Settings shards under qB/project static limits, embedded in the reusable candidate artifact, and that exact artifact is promoted/released without a main rebuild; Pages consumes hash-bound Frozen LKG + Locale facts, dispatches every exact dev/main HEAD so deploy-pages build identity and the public installer payload stay exact-SHA-bound, serializes external Pages writes without cancellation, re-checks current branch HEAD immediately before dev/main deployment, validates browser behavior through 16 bounded parallel Chrome jobs including a two-sim locale-bootstrap gate and ten stable-preference shards, recovers stale post-deploy matrix verification, main promotion reuses only the validated dev site artifact, and hosted Chrome remains canonical.`);
