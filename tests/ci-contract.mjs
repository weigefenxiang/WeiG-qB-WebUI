...
assert(pagesBuild.includes('test -s "$RUNNER_TEMP/virtual-qb-site/dev/app/__source/private/data/qb-releases.json"'),'Pages build must inject and verify the exact qB release catalog in product runtime data');
...
console.log(`CI contract passed for WeiG ${version}: exact qB 4.1.0+ source catalog is audited once on dev, executed through every formal product compatibility owner, compact-packed for qB WebUI static delivery, embedded in the reusable candidate artifact, and that exact artifact is promoted/released without a main rebuild; Pages consumes the hash-bound Frozen LKG without scheduled upstream re-extraction, main promotion reuses only the exact validated dev site artifact, and hosted Chrome remains canonical.`);
