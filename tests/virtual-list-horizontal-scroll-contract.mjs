...
assert.match(core,/__weigVirtualScrollIdleTimer=global\.setTimeout\(function\(\).*self\._flushPendingItems\(\);\},150\)/s,'the bounded quiet period must flush the newest pending snapshot after 150ms.');
...
const verticalTimer=el.__weigVirtualScrollIdleTimer;assert.ok(timers.has(verticalTimer));const beforeIdle=renders;timers.get(verticalTimer)();timers.delete(verticalTimer);assert.equal(list.items[0].id,4);assert.equal(renders,beforeIdle+1,'idle must add exactly one repaint for the newest pending snapshot');
el.scrollLeft=120;listeners.get('scroll')();list.setItems([{id:5}]);assert.equal(list.items[0].id,4,'horizontal motion must use the same scroll-priority owner');const horizontalTimer=el.__weigVirtualScrollIdleTimer;timers.get(horizontalTimer)();timers.delete(horizontalTimer);assert.equal(list.items[0].id,5);
list.destroy();assert.equal(el.__weigVirtualScrollHandler,null);assert.equal(el.__weigVirtualScrollIdleTimer,0);assert.equal(list._hasPendingItems,false);
