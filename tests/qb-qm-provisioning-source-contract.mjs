...
const entityLayered=mergeQbNativeQmRecoveryEvidence([
  buildQbNativeQmRecoveryEvidence([{qbVersion:'5.2.2',sourceSha:sha('7'),locale:'de',translationSource:ts('de',context('Ctx',scalar('Coalesce reads &amp;amp; writes','Zusammenführen'))) }]),
  buildQbNativeQmRecoveryEvidence([{qbVersion:'5.2.3',sourceSha:sha('8'),locale:'de',translationSource:ts('de',context('Ctx',scalar('Coalesce reads &amp; writes','Zusammenführen'))) }])
]);
const entityLayeredUnion=materializeQbNativeQmRecoveryUnion(entityLayered);
assert.deepEqual(entityLayeredUnion.locales.de.map(item=>item.source),['Coalesce reads & writes'],'multi-layer official TS source encodings must collapse into one canonical recovery identity before freezing');
