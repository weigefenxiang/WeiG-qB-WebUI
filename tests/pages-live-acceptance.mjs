...
try{
...

  {
    ...
    assert.equal(mainHeaderFacts.whiteSpace,'nowrap','Torrent header must never wrap vertically');
    assert.match(mainHeaderFacts.writingMode,/horizontal/i,'Torrent header must remain horizontal');

    const detailHash=hash;
    ...
    for(const surface of ['files','trackers','peers','webseeds']){
      ...
      assert.ok(facts.parts.length>=1,`${surface}: shared detail header must retain a visible grapheme`);
      assert.equal(facts.minDisplay,facts.parts[0],`${surface}: minimum detail header must retain the first grapheme through W.DataGridHeader: ${JSON.stringify(facts)}`);
      assert.equal(facts.aria,facts.full,`${surface}: detail header must retain full accessible label`);
      assert.equal(facts.whiteSpace,'nowrap',`${surface}: detail header must never wrap vertically`);
      assert.match(facts.writingMode,/horizontal/i,`${surface}: detail header must remain horizontal`);
      detailHeaderFacts.push(facts);
    }
    assert.deepEqual(detailHeaderFacts.map(item=>item.surface),['files','trackers','peers','webseeds'],'Content/Trackers/Users/HTTP Sources must all pass the same live DataGridHeader behavior');
