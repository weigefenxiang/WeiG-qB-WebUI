...
function semantic(world){return{trackerContract:resolveEndpointContract(world.profile,'torrents/trackers')};}
...

{
  ...
  const row=listTorrentsSnapshot(qB50,{limit:1,includeTrackers:'true',includeFiles:'true'},now,semantic(qB50))[0];
  assert.equal(Object.hasOwn(row,'trackers'),false,'qB 5.0.x must ignore later includeTrackers query option');
  assert.equal(Object.hasOwn(row,'files'),false,'qB 5.0.x must ignore later includeFiles option');
}

{
  const qB51=world('5.1.0','info-options-51');
  const row=listTorrentsSnapshot(qB51,{limit:1,includeTrackers:'true',includeFiles:'true'},now,semantic(qB51))[0];
  ...
  const off=listTorrentsSnapshot(qB51,{limit:1,includeTrackers:'false'},now,semantic(qB51))[0];
...

{
  ...
  const row=listTorrentsSnapshot(qB52,{hashes:withMeta.hash,includeTrackers:'true',includeFiles:'true'},now,semantic(qB52))[0];
  ...
  const pendingRow=listTorrentsSnapshot(qB52,{hashes:pending.hash,includeFiles:'true'},now,semantic(qB52))[0];
...

{
  ...
  const rows=listTorrentsSnapshot(qB52,{includeTrackers:'true',includeFiles:'true',limit:2,offset:1},now,semantic(qB52));
...
console.log('Virtual qB torrents/info option contract passed: includeTrackers begins at qB 5.1, includeFiles at qB 5.2, explicit Endpoint Contract semantics flow into tracker projection, and metadata gating matches upstream insertion behavior.');
