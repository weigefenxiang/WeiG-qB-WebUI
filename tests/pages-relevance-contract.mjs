...
for(const required of ['.github/workflows/pages-source.yml','.github/workflows/pages.yml','.github/workflows/ci.yml','.github/workflows/session-handshake.yml','tests/real-qb-session-race.sh','tools/product-identity.mjs','tools/qb-settings-*.mjs','tools/qb-native-qm-recovery.mjs','tools/qb-detail-surface-parsers.mjs','tools/qb-torrent-fields-parser.mjs','tools/qb-webui-catalog.mjs'])assert.ok(matcher.includes(required),`Pages relevance boundary missing ${required}`);
