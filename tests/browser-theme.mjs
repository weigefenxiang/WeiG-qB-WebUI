...
try{
  ...
  // Feature-specific Light surfaces sampled on modern qB: Transfer and Logs.
  {
    ...
    await page.locator('#app-nav [data-route="logs"]').click();await page.waitForFunction(()=>document.getElementById('logs-view')?.classList.contains('is-active'));await page.waitForSelector('.logs-toolbar');await page.waitForSelector('#logs-size-mode .ui-select__trigger');const logsToolbar=await surface(page,'.logs-toolbar'),logsSize=await surface(page,'#logs-size-mode .ui-select__trigger');assert(whiteBased(logsToolbar)&&whiteBased(logsSize),`modern: Light Logs surfaces remained Dark`);await context.close();
  }
