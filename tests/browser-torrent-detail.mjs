...
const torrent={hash,name:'Detail source browser fixture',size:64*1024*1024,progress:.25,dlspeed:65536,upspeed:2048,eta:900,state:'downloading',ratio:.5,tracker:'https://tracker.example/announce?token=exact',category:'Detail',tags:'source',num_seeds:5,num_leechs:2,save_path:'/downloads',added_on:1700000000,completion_on:0,priority:1,private:false};
...
async function setColumnVisible(page,key,visible){
  ...
  await page.waitForSelector('#shared-column-dialog[open]');
  ...
  await page.locator('#shared-column-dialog .dialog__actions button').last().click();
