...
try{
  ...
  for(const expected of [{name:'clip.mp4',type:'video',marker:'VID'},{name:'photo.jpg',type:'image',marker:'IMG'},{name:'installer.exe',type:'app',marker:'APP'},{name:'notes.txt',type:'text',marker:'TXT'}]){const hit=fileIconPresentation.find(item=>item.name===expected.name);assert(hit&&hit.type===expected.type&&hit.marker===expected.marker&&hit.glyph==='',`Content file icon classifier mismatch for ${expected.name}: ${JSON.stringify(fileIconPresentation)}`);}
