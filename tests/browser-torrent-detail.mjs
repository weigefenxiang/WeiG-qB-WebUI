...
const torrent={hash,name:'Detail source browser fixture — '+('long title ownership '.repeat(12)),size:64*1024*1024,progress:.25,dlspeed:65536,upspeed:2048,eta:900,state:'downloading',ratio:.5,tracker:'https://tracker.example/announce?token=exact',category:'Detail',tags:'source',num_seeds:5,num_leechs:2,save_path:'/downloads',added_on:1700000000,completion_on:0,priority:1,private:false};
...
try{
  ...
  const headerLayout=await page.evaluate(()=>{const stack=document.querySelector('.detail-status-stack'),state=document.getElementById('detail-state'),progress=document.querySelector('.detail-status-stack .detail-progress'),track=progress&&progress.querySelector('.progress-track'),pct=document.getElementById('detail-progress-text'),title=document.getElementById('detail-title');const box=x=>{const r=x.getBoundingClientRect();return{x:r.x,y:r.y,width:r.width,height:r.height,right:r.right,bottom:r.bottom,cy:r.y+r.height/2};};return{sameOwner:!!(stack&&state&&progress&&stack.contains(state)&&stack.contains(progress)),stack:box(stack),state:box(state),progress:box(progress),track:box(track),pct:box(pct),title:box(title),text:pct.textContent};});
  assert(headerLayout.sameOwner,'Detail status and progress do not share the canonical right-status owner.');
  assert(headerLayout.state.bottom<=headerLayout.progress.y+1,'Desktop Detail status must sit above progress: '+JSON.stringify(headerLayout));
  assert(Math.abs(headerLayout.state.right-headerLayout.stack.right)<2&&Math.abs(headerLayout.progress.right-headerLayout.stack.right)<2,'Desktop Detail status stack is not right-aligned: '+JSON.stringify(headerLayout));
  assert(Math.abs(headerLayout.track.cy-headerLayout.pct.cy)<3&&headerLayout.text==='25%','Detail progress track and percentage must share one row: '+JSON.stringify(headerLayout));
  assert(headerLayout.title.right<=headerLayout.stack.x+1,'Long Detail title overlaps the right-status stack: '+JSON.stringify(headerLayout));
