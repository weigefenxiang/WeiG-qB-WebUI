...
const qbPromote=settings.indexOf('controller.prefs=result.prefs||{}',qbExecute);
assert(qbWrite>=0&&qbVerifyRead>qbWrite&&qbExecute>=0&&qbPromote>qbExecute,'qB Save must retain transaction-owned write plus verification readback before promoting verified Preferences');
...
assert(settingsSchema.includes('function nativeSurfaces()')&&settingsSchema.includes('function group(tab,prefs)')&&settingsSchema.includes('function sourcePreference(key)')&&!settingsSchema.includes("add('speed'"),'SettingsSchema must expose source-native tab/group/preference projection without a fixed Speed allowlist');
assert(settingsSchema.includes('function nativeSurfaces()')&&settings.includes('function syncQbTabs()')&&settings.includes('S.nativeSurfaces&&S.nativeSurfaces()')&&!settings.includes('QB_TAB_ORDER')&&!settings.includes('function ensureQbTabs()'),'Settings owner must project native qB tabs from SettingsSchema source truth without restoring the retired fixed-tab reconciler');
...
assert(ui.includes("rssOpenButton.id='rss-add-open-btn'")&&ui.includes("W.DialogRuntime.create({id:'rss-add-dialog'")&&ui.includes('actions.append(rssOpenButton,refresh)'),'RSS Add Feed and Refresh must occupy the title header action rail while the dialog consumes canonical DialogRuntime');
...
assert(transfer.includes('compactChart=null,chartWindow=300')&&transfer.includes('function setChartWindowMinutes(value)')&&transfer.includes('chartWindow=minutes*60'),'Drawer and full Transfer dialog must share one 5-minute default and the canonical editable minute owner');
