...
assert(transferCss.includes('#sidebar{display:grid;grid-template-rows:minmax(0,1fr) auto auto;align-content:stretch;overflow:hidden}')&&transferCss.includes('.mobile-drawer-telemetry{display:grid!important;grid-row:2;order:20')&&transferCss.includes('.sidebar__meta{grid-row:3;order:30'),'Mobile Drawer must reserve an independent scroll row above telemetry and keep qB/WebAPI version metadata in the final row');
