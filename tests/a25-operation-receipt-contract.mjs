...
assert.match(selection,/actionReceipt\(kind,value,verified===true,hashes\)/);
assert.match(selection,/setSuperSeeding[\s\S]*?actionReceipt\(kind,'',false,hashes\)/);
assert.ok(!selection.includes("verified===false?false:true"),'unverified mutation must never be promoted to verified success');
