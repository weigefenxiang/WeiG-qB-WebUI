...
usage() {
  cat <<'EOF'
Usage:
  sh tests/live-v036.sh --sha <40-char-git-sha> --target <installed-webui-dir> [--target <installed-webui-dir> ...]

Example:
  sh tests/live-v036.sh \
    --sha <40-char-git-sha> \
    --target /srv/qb-a/config/weigg-qb-webui \
    --target /srv/qb-b/config/weigg-qb-webui
...
update_metadata() {
  ...
  [ -f "$file" ] || return 0
  sed -i -E \
    -e 's#("version"[[:space:]]*:[[:space:]]*")[^"]*(")#\10.3.6\2#' \
    -e "s#(\"gitSha\"[[:space:]]*:[[:space:]]*\")[^\"]*(\")#\\1${SHA}\\2#" \
    -e "s#(\"installedAt\"[[:space:]]*:[[:space:]]*\")[^\"]*(\")#\\1${NOW}\\2#" \
    -e 's#("installer"[[:space:]]*:[[:space:]]*")[^"]*(")#\1live-v036-test\2#' \
    "$file"
}
...
prepare_target() {
  ...
  [ "$(tr -d '\r\n ' < "$new/VERSION")" = "0.3.6" ] || { rm -rf "$new"; echo "ERROR: staged VERSION invalid: $dest" >&2; return 1; }
...
while IFS= read -r dest; do
  [ -n "$dest" ] || continue
  if ! prepare_target "$dest"; then exit 1; fi
done < "$TARGET_FILE"
