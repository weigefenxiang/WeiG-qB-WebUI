...

{
  ...
  response=await handleApi(w,formRequest('rss/setRule',{ruleName:'Modern Shape',ruleDef:JSON.stringify({enabled:true,mustContain:'Nested only',torrentParams:{category:'Modern',tags:['nested-rss'],save_path:'/modern',stopped:true,content_layout:'Subfolder'}})}));
  assert.equal(response.status,200,'modern RSS rule shape must be writable without flattening torrentParams');
  const modernRules=await (await handleApi(w,getRequest('rss/rules'))).json();
  assert.deepEqual(modernRules['Modern Shape'].torrentParams,{category:'Modern',tags:['nested-rss'],save_path:'/modern',stopped:true,content_layout:'Subfolder'},'Virtual RSS rule reread must preserve modern torrentParams exactly for UI writeback validation');
