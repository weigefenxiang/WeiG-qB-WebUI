...
try{
  {
    ...
    assert(mobileLayout.pageWidth<80,`page-size Select is not intrinsic (${mobileLayout.pageWidth}px)`);assert(mobileLayout.chevron==='none','Select chevron must be visually removed');
    ...
    const about=await page.evaluate(()=>({one:document.querySelectorAll('#settings-content .about-surface').length,upstream:document.querySelector('.about-attribution')?.textContent||'',cards:document.querySelectorAll('#settings-content .about-surface .setting-card').length}));
    assert(about.one===1,'About must be one coherent surface');assert(/Christophe Dumez/.test(about.upstream),'About upstream attribution missing');assert(about.cards===0,'About must not remain a mosaic of setting cards');
  ...
  }
  {
    ...
    await page.goto(`http://${host}:${port}/#/`,{waitUntil:'networkidle'});await page.waitForFunction(()=>!!WeiG.V037?.ui&&!!document.querySelector('#transfer-capsule'),null,{timeout:7000});await page.waitForTimeout(1100);
    const status=await page.evaluate(()=>({timezone:document.querySelectorAll('[data-status-timezone]').length,capsule:!!document.querySelector('#transfer-capsule #status-dl')&&!!document.querySelector('#transfer-capsule #status-up'),mode:document.querySelector('#transfer-capsule')?.dataset.mode}));
    assert(status.timezone===0,'desktop statusbar timezone must be removed');assert(status.capsule,'Transfer Capsule must contain both download and upload rates');assert(status.mode==='normal','fixture should start in NORMAL transfer mode');
    await page.locator('#transfer-capsule').click();await page.waitForSelector('#v037-transfer-dialog[open]');
    const transfer=await page.evaluate(()=>({normal:document.querySelector('[data-transfer-mode="normal"]')?.classList.contains('is-active'),inputs:document.querySelectorAll('#v037-transfer-dialog [data-transfer-rate]').length,unit:!!document.querySelector('#v037-transfer-dialog .transfer-unit-select')}));
    assert(transfer.normal&&transfer.inputs===2&&transfer.unit,'Transfer editor must expose mode, both rates and unit');
