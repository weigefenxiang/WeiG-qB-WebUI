...
try{
  {
    ...
    await page.locator('#mobile-bottom-nav [data-route="settings"]').click();await page.waitForFunction(()=>WeiG.Router.route().name==='settings');await page.waitForSelector('#settings-content .settings-grid-canonical');
    const mobileSettings=await page.evaluate(()=>{const grid=document.querySelector('#settings-content .settings-grid-canonical'),rows=Array.from(grid?.querySelectorAll(':scope>.setting-row-grid')||[]),tz=document.querySelector('[data-v036-timezone]'),cols=grid?getComputedStyle(grid).gridTemplateColumns:'';return {rows:rows.length,cols,max:Math.max(...rows.slice(0,8).map(x=>x.getBoundingClientRect().height)),timezone:!!tz};});
...

  {
    ...
    await page.evaluate(()=>WeiG.Router.go('settings'));await page.waitForFunction(()=>WeiG.Router.route().name==='settings');await page.locator('#brand-btn>.brand-name-home').click();await page.waitForFunction(()=>WeiG.Router.route().name==='home');
    await page.evaluate(()=>WeiG.Router.go('settings'));await page.waitForFunction(()=>WeiG.Router.route().name==='settings');await page.locator('#brand-btn>.brand-mark-home').click();await page.waitForFunction(()=>WeiG.Router.route().name==='home');
    ...
    await page.locator('#app-nav [data-route="settings"]').click();await page.waitForFunction(()=>WeiG.Router.route().name==='settings');await page.waitForSelector('#settings-content .settings-grid-canonical');
    const weigg=await page.evaluate(()=>{const grid=document.querySelector('#settings-content .settings-grid-canonical'),rows=Array.from(grid?.querySelectorAll(':scope>.setting-row-grid')||[]).filter(x=>x.offsetParent!==null),rects=rows.slice(0,6).map(x=>x.getBoundingClientRect()),controls=rows.slice(0,6).map(row=>{const c=row.querySelector('.switch-control,.field-input,.ui-select,select'),r=row.getBoundingClientRect(),cr=c?.getBoundingClientRect();return cr?Math.abs(r.right-cr.right):999;});return {rows:rows.length,cols:getComputedStyle(grid).gridTemplateColumns,tops:rects.map(r=>Math.round(r.top)),lefts:rects.map(r=>Math.round(r.left)),rightDelta:Math.max(...controls)};});
    ...
    await page.locator('#settings-tabs [data-settings-tab="connection"]').click();await page.waitForSelector('#settings-content .settings-grid-canonical');
    ...
    await page.locator('#settings-tabs [data-settings-tab="advanced"]').click();await page.waitForSelector('#settings-content .settings-grid-canonical .setting-row-grid');
    const scroll=await page.evaluate(()=>{const scroller=document.getElementById('settings-content');scroller.scrollTop=scroller.scrollHeight;const rows=Array.from(scroller.querySelectorAll('.setting-row-grid')).filter(x=>x.offsetParent!==null),last=rows.at(-1);return {scrollHeight:scroller.scrollHeight,clientHeight:scroller.clientHeight,scrollTop:scroller.scrollTop,lastBottom:last?.getBoundingClientRect().bottom||0,viewportBottom:scroller.getBoundingClientRect().bottom};});
