...
try{
...

  {
    ...
    const refreshNoise=/^(已刷新|refreshed|refresh complete)$/i.test(status.statusMessage);
    assert(!refreshNoise&&status.bottomDelta<4&&status.connection&&status.centerDelta<16,`desktop StatusDock geometry regressed ${JSON.stringify(status)}`);
