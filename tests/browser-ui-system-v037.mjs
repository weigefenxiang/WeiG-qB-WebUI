...
try{
...

  {
    ...
    await page.locator('#settings-tabs [data-settings-tab="connection"]').click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('#settings-content .setting-row-grid')).some(x=>x.offsetParent!==null));
    const qb=await page.evaluate(()=>{const visibleRow=Array.from(document.querySelectorAll('#settings-content .setting-row-grid')).find(x=>x.offsetParent!==null),grid=visibleRow?.closest('.settings-grid-canonical'),rows=Array.from(grid?.querySelectorAll(':scope>.setting-row-grid')||[]).filter(x=>x.offsetParent!==null),r=rows.slice(0,4).map(x=>x.getBoundingClientRect());return {rows:rows.length,panel:!!grid?.closest('.settings-section-panel'),two:r.length>=2&&Math.abs(r[0].top-r[1].top)<2&&r[1].left>r[0].left+20};});
    assert(qb.rows>=4&&qb.panel&&qb.two,`qB Connection must use the same two-column Settings mother template (${JSON.stringify(qb)})`);

    await page.locator('#settings-tabs [data-settings-tab="advanced"]').click();await page.waitForFunction(()=>Array.from(document.querySelectorAll('#settings-content .setting-row-grid')).some(x=>x.offsetParent!==null));
