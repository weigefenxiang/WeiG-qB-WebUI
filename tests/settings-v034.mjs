...
for(const token of ['css/settings-v034.css?v=0.3.4','scripts/i18n-v034.js?v=0.3.4','scripts/settings-v034.js?v=0.3.4','data-i18n="tag.all"'])assert(index.includes(token),`v0.3.4 index token missing: ${token}`);
const runtime=fs.readFileSync('webui/private/scripts/settings-v034.js','utf8');
for(const token of ['alternative_webui_enabled','alternative_webui_path','getPreferences','setPreferences','weigg-install.json','hostPath','qbPath','global.confirm','global.location.href=\'/\'','v034SaveBridge'])assert(runtime.includes(token),`v0.3.4 runtime token missing: ${token}`);
