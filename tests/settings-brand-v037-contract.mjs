...
for(const token of ['BRAND-SYSTEM-001','BRAND-IDENTITY-001','SETTINGS-DESIGN-001','W.Navigation.goHome','BrandMark','BrandCluster','BrandIdentity','ControlRegistry','settings-grid-canonical','settingSpan','settingsOwner','activeSettingsOwner','mergeExtraGrids','decorateRow'])assert(js.includes(token),`Settings/Brand canonical owner missing ${token}`);
...
assert(js.includes("querySelectorAll('.settings-row,.settings-row--canonical,.settings-control')"),'SettingsGrid must recognize native qB rows directly instead of waiting for another observer to add canonical classes');
...
assert(js.includes("if(owner==='weigg'&&index>0)sectionOwner='weigg-metrics'"),'WeiG diagnostic section must not impersonate the editable interface grid');
...
assert(!js.includes('settleSettings'),'Settings normalization must be mutation-driven, not repaired by timer retries');
...
for(const token of ['--settings-content-max:1240px','--settings-grid-breakpoint:1180px','repeat(2,minmax(0,1fr))','grid-template-columns:minmax(0,1fr) auto','justify-self:end!important','data-setting-span="full"','--brand-mark-identity:50px','brand-mark-home','brand-name-home','brand-identity__mark-home','@media(max-width:1180px)','@media(prefers-reduced-motion:reduce)'])assert(css.includes(token),`Settings/Brand CSS contract missing ${token}`);
