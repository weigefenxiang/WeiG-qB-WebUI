...
const entitySource='I2P inbound quantity (requires libtorrent >= 2.0):';
assert.equal(canonicalQbSourceText('I2P inbound quantity (requires libtorrent &gt;= 2.0):'),entitySource);
assert.equal(canonicalQbSourceText('I2P inbound quantity (requires libtorrent &amp;gt;= 2.0):'),entitySource);
assert.equal(canonicalQbSourceText('Coalesce reads &amp;amp; writes (requires libtorrent &amp;lt; 2.0):'),'Coalesce reads & writes (requires libtorrent < 2.0):');
assert.equal(canonicalQbDisplayText('I2P 传入量 （需要 libtorrent &amp;gt;= 2.0）：'),'I2P 传入量 （需要 libtorrent >= 2.0）：');
assert.equal(canonicalQbSourceText('A & B'),'A & B');
assert.equal(canonicalQbSourceText('<![CDATA[A &amp; B &#62; &#x3d; C]]>'),'A & B > = C');
assert.equal(qbSourceRefKey('OptionsDialog','A &amp;amp; B'),'OptionsDialog\u0000A & B');
assert.deepEqual(auditQbEntityText('A &amp;amp; B'),{hadEntities:true,layers:2,canonical:'A & B',unresolved:[]});
const entityTs=`<TS version="2.1" language="zh_CN"><context><name>OptionsDialog</name>
<message><source>I2P inbound quantity (requires libtorrent &amp;gt;= 2.0):</source><translation>I2P 传入量 （需要 libtorrent &amp;gt;= 2.0）：</translation></message>
<message><source>Coalesce reads &amp;amp; writes (requires libtorrent &amp;lt; 2.0):</source><translation>合并读写（需要 libtorrent &amp;lt; 2.0）：</translation></message>
</context></TS>`;
const entityParsed=parseQtTsTranslationSource(entityTs,{contexts:['OptionsDialog']});
assert.deepEqual(entityParsed.messages.map(item=>item.source),[entitySource,'Coalesce reads & writes (requires libtorrent < 2.0):']);
assert.equal(entityParsed.messages[0].translation,'I2P 传入量 （需要 libtorrent >= 2.0）：');
assert.equal(entityParsed.messages[1].translation,'合并读写（需要 libtorrent < 2.0）：');
