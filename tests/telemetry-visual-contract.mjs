...
assert(transfer.includes("var last=null,samples=[],minuteBuckets=[],RAW_MAX=900,BUCKET_MAX=720")&&transfer.includes('function averageSamples(data,seconds)')&&transfer.includes("{value:'3',label:'3 s'}")&&transfer.includes("{value:'5',label:'5 s'}")&&transfer.includes("{value:'10',label:'10 s'}")&&transfer.includes("{value:'15',label:'15 s'}")&&transfer.includes("{value:'20',label:'20 s'}")&&transfer.includes("{value:'30',label:'30 s'}")&&transfer.includes('chartAverage=10'),'Transfer smoothing must remain display-only over the existing bounded history with Raw/3s/5s/10s/15s/20s/30s and default 10s averaging');
...
console.log('Telemetry visual contract passed: bounded display-only averaging with default 10s plus 15/20/30s options, session totals and realtime rates share canonical legend colors, slower Connected breathing, Android typography, and Reduced Motion protection.');
