...
assert.ok(artifactResolver.includes("const sourceSha=arg('source-sha'")&&artifactResolver.includes("process.argv.includes('--certified-only')"),'Settings translation resolver must support exact source-SHA and certified-only evidence selection');
assert.ok(artifactResolver.includes('const artifacts=await listArtifacts({maxPages:ARTIFACT_MAX_PAGES});')&&!artifactResolver.includes('POLL_INTERVAL_MS')&&!artifactResolver.includes('sleep(')&&!artifactResolver.includes('dispatch-if-missing'),'Settings translation resolver must perform one bounded evidence read and never poll, sleep, or dispatch evidence work itself');
assert.ok((artifactResolver.match(/await tryBootstrapCatalog\(artifacts\)/g)||[]).length===1&&artifactResolver.includes('if(!certifiedOnly&&await tryBootstrapCatalog(artifacts))'),'bootstrap reuse may remain available to explicit non-certified callers, while Pages can require the already-frozen exact-SHA LKG');
assert.ok(artifactResolver.includes('incompatibleArtifactIds'),'single-pass evidence scanning must still avoid repeatedly attempting incompatible artifacts within one resolution');
...
assert.equal(lkg.trackerFilterBindings,4,'LKG v2 must freeze exact source-derived Tracker filter facts across profiles');
...
const staleRawRecovery=structuredClone(lkg);staleRawRecovery.recovery.union.locales.de[0].source='Language&amp;:';staleRawRecovery.recovery.unionSha256=hash(staleRawRecovery.recovery.union);
assert.throws(()=>applyQbSettingsTranslationLkg(frozen,staleRawRecovery),/must use canonical context\/source/,'artifact reuse must reject pre-canonical recovery union messages before Pages materialization');
const duplicateRecovery=structuredClone(lkg);duplicateRecovery.recovery.union.locales.de.push(structuredClone(duplicateRecovery.recovery.union.locales.de[0]));duplicateRecovery.recovery.unionSha256=hash(duplicateRecovery.recovery.union);
assert.throws(()=>applyQbSettingsTranslationLkg(frozen,duplicateRecovery),/duplicate canonical recovery union message/,'artifact reuse must reject canonical duplicate recovery messages instead of deferring failure to runtime bundle construction');
