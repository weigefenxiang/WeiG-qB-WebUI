...
assert.ok(artifactResolver.includes('lkg?.schemaVersion!==2')&&artifactResolver.includes('torrentTableColumns')&&artifactResolver.includes('recoveryRoutes'),'certified artifact reuse must reject stale v1 evidence and require native columns plus recovery coverage');
...
const sourceProfile=(qbVersion,sourceSha,sets)=>({qbVersion,sourceSha,webuiLocales:[{value:'en'},{value:'de'}],settingsUiSource:'qb-upstream-preferences-ui',settingsUiMappedPreferences:1,settingsUiTotalPreferences:1,settingsUi:{locale:{controlId:'locale_select',title:{source:'Language:',context:'OptionsDialog'}}},qbOwnedUiSource:'qb-upstream-webui-source-context',qbOwnedUi:{'column.name':{source:'Name',context:'TransferListModel'}},settingsTranslations:{en:hEn,de:hDe},settingsTranslationSets:sets,torrentTableColumns:[nativeColumn('name','Name'),nativeColumn('size','Size',false)]});
...
assert.equal(lkg.schemaVersion,2,'Settings/source LKG must advance to schema v2 before native column/recovery facts can be certified');
