...
assert.ok(qbSource.includes('R=W.CapabilityRegistry')&&qbSource.includes("method==='POST'&&!/^auth\\//.test(String(path||''))")&&qbSource.includes("typeof R.isCertified!=='function'||!R.isCertified()"),'QBClient transport must fail closed through CapabilityRegistry for non-auth POST writes when the current release is not exact/equivalent certified.');
assert.equal(qbSource.includes('W.ReleaseProfile'),false,'QBClient must not bypass CapabilityRegistry for release/source compatibility facts.');
...
assert.equal(logoutMatches.length,1,'Logout must remain one narrowly scoped POST auth/logout void call with no payload so users can terminate a session even when the compatibility registry is unavailable.');
console.log(`Compatibility architecture contract passed: scanned ${files.length} complete Git-indexed product script blobs; CapabilityRegistry owns feature/version capability materialization, QBClient has no direct ReleaseProfile or qB major/version fallback branches, API transport remains centralized, non-auth POST writes remain source-owned, ${directPosts.length} direct POST method(s) have reviewed ownership, and ${legacySeen.length} remaining legacy owner blob(s) stay frozen for explicit migration/review.`);
