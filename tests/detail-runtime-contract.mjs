...
assert.match(ui,/function detailValueSource\(surface,column\).*if\(key&&keys\.indexOf\(key\)>=0\)return'response';.*return scalar\.length===1\?'projection':'derived';/s,'detail runtime must distinguish direct response columns, bounded source projections, and derived UI values');
assert.match(ui,/function propertyValue\(surface,item,column\).*if\(valueSource==='derived'\)return derivedDetailValue\(surface,item,column\);.*if\(valueSource==='projection'\)return projectedDetailValue\(item,column\);.*if\(keys\.indexOf\(key\)<0\|\|!owns\(item,key\)\)return\{key:key,value:null\};/s,'response columns must read only their proven key, projections must use one proven dependency, and derived columns must use the bounded resolver');
