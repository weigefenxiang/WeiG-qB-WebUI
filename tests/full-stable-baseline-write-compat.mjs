...
for(const profile of catalog){
  ...
  const addDesc=profile.apiActionParameters?.[ADD];assert.ok(addDesc,`${profile.qbVersion}: Add Torrent parameter descriptor missing`);assert.ok(addDesc.parameters.includes('urls')&&addDesc.parameters.includes('savepath'),`${profile.qbVersion}: Add Torrent must source-prove urls/savepath`);const stoppedParam=addDesc.parameters.includes('stopped')?'stopped':(addDesc.parameters.includes('paused')?'paused':'');assert.ok(stoppedParam,`${profile.qbVersion}: Add Torrent must source-prove paused/stopped state parameter`);
  ...
  seen=[];client.request=async(reqPath,options={})=>{seen.push({path:reqPath,options});return 'Ok.';};const result=await client.add('magnet:?xt=urn:btih:abc',[],'/downloads',{[stoppedParam]:'false'});assert.equal(result,'Ok.');assert.equal(seen.length,1);assert.equal(seen[0].path,'torrents/add');assert.equal(seen[0].options.method,'POST');assert.equal(seen[0].options.body.get('urls'),'magnet:?xt=urn:btih:abc');assert.equal(seen[0].options.body.get('savepath'),'/downloads');assert.equal(seen[0].options.body.get(stoppedParam),'false');assert.equal(seen[0].options.body.has(stoppedParam==='stopped'?'paused':'stopped'),false,`${profile.qbVersion}: Add Torrent must not send the opposite generation stop parameter`);dispatchChecks++;
}
