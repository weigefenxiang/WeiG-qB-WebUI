...
assert.ok(ci.includes(generated)&&ci.includes(enriched)&&ci.includes('--shard-count=12')&&ci.includes(merged)&&ci.includes(audited),'candidate pipeline must generate, fan out source enrichment, merge, and audit the exact catalog');
assert.ok(ci.includes('release_locale_enrich:')&&ci.includes('needs: release_catalog_base')&&ci.includes('release_catalog_merge:')&&ci.includes('needs: release_locale_enrich')&&ci.includes('release_product_matrix:')&&ci.includes('needs: release_catalog_merge'),'qB locale and Settings UI facts must flow through the base -> 12-way enrich -> merge -> product DAG');
