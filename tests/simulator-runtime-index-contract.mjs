...

{
  ...
  assert.equal(before.advanceRuns,1,'fixture must enter a real two-second runtime bucket before testing same-bucket controls');
  ...
  assert.equal(after.advanceRuns,1,'same-bucket rate-control change must not advance simulation time again');
