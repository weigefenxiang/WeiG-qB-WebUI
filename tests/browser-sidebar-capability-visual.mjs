...
async function waitReady(page){await page.waitForSelector('#logout-btn');await page.waitForFunction(()=>window.WeiG?.CapabilityRegistry?.releaseIdentity()?.qbVersion&&window.WeiG?.CapabilityRegistry?.state('tags')?.feature&&window.WeiG?.AppState?.client?.qbVersion&&window.WeiG.AppState.client.qbVersion!=='0.0.0'&&window.WeiG?.AppState?.catalogReady===true);await page.waitForFunction(()=>document.querySelectorAll('[title],[data-tooltip]').length===0,{timeout:5000});}
