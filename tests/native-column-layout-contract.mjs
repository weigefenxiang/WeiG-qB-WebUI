...
const hardMinOwner=coreSource.slice(coreSource.indexOf('function gridColumnHardMin'),coreSource.indexOf('function gridFitHeaderCell'));assert.equal(hardMinOwner.includes("state_icon"),false,'header sizing must remain semantic/shared and must not special-case state_icon.');assert.ok(coreSource.includes('function gridColumnAlign(column)')&&coreSource.includes("key==='state_icon'||key==='checked'"),'one shared DataGrid geometry owner must align icon/checkbox columns without feature-local painters.');
