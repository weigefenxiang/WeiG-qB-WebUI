...
active=F.effectiveDesktopColumns(saved,profile);
assert.ok(active.findIndex(x=>x.key==='upspeed')<active.findIndex(x=>x.key==='dlspeed'),'visible user order override must support swapping upload/download columns');assert.deepEqual(Array.from(F.sourceOrderedColumnDefinitions(profile),x=>x.key),profile.torrentTableColumns.map(x=>x.key),'ColumnConfigurator presentation order must stay on exact qB source order even after the table itself is user-reordered');
...
assert.ok(columnSource.includes("shared-column-settings__label")&&columnSource.includes("row.addEventListener('click'")&&columnSource.includes("check.dispatchEvent(new Event('change',{bubbles:true}))"),'ColumnConfigurator rows must toggle visibility from the whole non-control row surface, not only the tiny checkbox.');
assert.ok(tableCss.includes('width:min(var(--column-config-dialog-width,900px),calc(100vw - 24px))')&&tableCss.includes('[data-column-layout="2"] .shared-column-settings{grid-template-columns:repeat(2,minmax(0,1fr))}')&&tableCss.includes('[data-column-layout="3"] .shared-column-settings{grid-template-columns:repeat(3,minmax(0,1fr))}')&&tableCss.includes('font-size:calc(var(--text-body) + 2px)'),'ColumnConfigurator desktop layout must choose a compact two/three-column geometry with readable row text instead of fixed sparse columns.');
assert.ok(columnSource.includes('twoHeight<=available+1?2:Math.min(3,count)')&&columnSource.includes("--column-config-dialog-width")&&columnSource.includes("longest=Math.max(longest,label.scrollWidth||0)"),'ColumnConfigurator must keep two columns when all rows fit without body scrolling, escalate to three only when needed, and size the dialog from the longest visible label.');
assert.ok(appSource.includes('reorderable:false')&&uiSource.includes('reorderable:false')&&columnSource.includes('controls.hidden=!reorderable'),'source-ordered main/detail configurators must keep option positions stable and hide the contradictory up/down controls while table-header drag order remains independent.');
...
const hardMinOwner=coreSource.slice(coreSource.indexOf('function gridColumnHardMin'),coreSource.indexOf('function gridFitHeaderCell'));assert.equal(hardMinOwner.includes("state_icon"),false,'header sizing must remain semantic/shared and must not special-case state_icon.');assert.ok(coreSource.includes('function gridColumnAlign(column)')&&coreSource.includes("var requested=String(column&&column.align||'').toLowerCase()")&&coreSource.includes("key==='state_icon'||key==='checked'")&&coreSource.includes("if(key==='progress')return'stretch';return'start'"),'one shared DataGrid geometry owner must default ordinary text/numeric columns to left/start while reserving special geometry for explicit schema/icon/progress columns.');assert.equal(coreSource.includes('(?:size|speed|eta|ratio|port|downloaded|uploaded|remaining|peers|seeds|leeches|priority|availability|time|date|count|limit)$'),false,'numeric-looking field names must no longer silently create a second right-alignment policy.');
...
assert.ok(uiSource.includes("return['files','trackers','peers','webseeds'].indexOf(tab)>=0?tab:''")&&uiSource.includes('function applyDetailColumns(ctx,rebuild)'),'Content, Trackers, Users/Peers and HTTP Sources/WebSeeds must all enter the same generic detail-table header path.');assert.ok(uiSource.includes('function detailWidth(column)')&&uiSource.includes('function sourceColumnKeys(column)')&&uiSource.includes('function detailValueSource(surface,column)'),'shared Detail column projection helpers must remain owned beside the generic Detail surface path; FlagProvider edits must not delete them.');
...
assert.ok(!coreSource.includes('onChange(cols,false)'),'pointermove resize must not rebuild the DataViewport or persist state on every frame');
assert.ok(coreSource.includes("if(!vertical||self._rendering||self.el.__weigDataViewportScrollFrame)return")&&coreSource.includes('self.render(false)'),'pure horizontal native scrollbar motion must bypass DataViewport rendering while vertical work is coalesced to animation frames');
assert.ok(appSource.includes('app.viewport.commitWhenIdle(finalizeListUi)')&&coreSource.includes("classList.add('is-scroll-interacting')")&&coreSource.includes("classList.remove('is-scroll-interacting')"),'polling-side DOM synchronization and progress presentation must yield to the one canonical DataViewport interaction owner.');
assert.ok(tableCss.includes('.data-grid__head>[data-align=start],.data-grid__row>[data-align=start]{justify-content:flex-start;text-align:left}')&&tableCss.includes('#torrent-list>.torrent-table-head{position:sticky;top:0;z-index:8;width:max-content;min-width:100%;background:var(--bg-surface)'),'shared table alignment must be left/start by default and the sticky Torrent header must paint an opaque canonical surface.');
...
assert.ok(coreSource.includes("this.staticHead=options.staticHead||null")&&appSource.includes("staticHead:U.$('torrent-table-head')"),'Torrent main must explicitly hand its header to the canonical DataViewport instead of relying on an id-specific constructor branch.');
...
assert.ok(layoutSource.includes("var TABLE_COLUMN_KEY=(W.StorageKeys&&W.StorageKeys.tableColumnsPrefix)||'weig.tableColumns:'")&&layoutSource.includes('W.SharedColumns={resolve:resolveColumns,commit:commitColumns,readSort:readSort,commitSort:commitSort,reset:resetColumns,read:tableState'),'all reusable detail tables must share one stable schema-backed column/sort state owner');
...
assert.ok(layoutSource.includes("if(!key||key==='checked'||!source.some")&&layoutSource.includes('state.sort={key:key,reverse:reverse===true}')&&uiSource.includes("persistedSort=W.SharedColumns.readSort?W.SharedColumns.readSort(tableId,source)")&&uiSource.includes('W.SharedColumns.commitSort(ctx.tableId,ctx.source,ctx.sortKey,ctx.sortReverse)'),'Detail sort key/direction must persist through the same per-surface SharedColumns state owner and reject stale source keys');
...
assert.ok(appSource.includes('C.torrentRow(t,app.selection.has(t.hash),handlers,app.columns,W.DataGrid.template(app.columns))'),'DataViewport rows must render from the current column order/width instead of a stale captured template');
