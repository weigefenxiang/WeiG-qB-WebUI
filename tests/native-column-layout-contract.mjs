...
assert.ok(layoutSource.includes("function exactDetailUi(){var R=W.ReleaseProfile,ui=R&&typeof R.torrentDetailUi==='function'?R.torrentDetailUi():null;")&&!layoutSource.includes('profile&&profile.torrentDetailUi'),'detail UI evidence must be consumed only through ReleaseProfile.torrentDetailUi(), whose owner enforces exact/equivalent certification and fallback closure');
