...
assert.ok(coreSource.includes('var GRID_MIN_WIDTH=24')&&coreSource.includes('function paintVisibleRows()')&&coreSource.includes("U.$$('.torrent-row').forEach"),'DataGrid resize must paint the current header and visible rows immediately at the compact width floor');
...
assert.ok(coreSource.includes('function commitReorder(sourceKey,targetKey,before)')&&coreSource.includes("e.target.closest('.col-resize')")&&coreSource.includes('cell.__weiggSuppressSort'),'direct Desktop header reorder must share the same DataGrid columns while preserving resize and ordinary sort clicks');
...
console.log('Native column layout contract passed: exact qB schema/defaults remain authoritative, user columns can shrink to 24px, header reorder shares the same state, and live resize avoids per-frame VirtualList rebuilds.');
