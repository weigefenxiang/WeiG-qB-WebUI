...
assert.ok(appSource.includes("row.addEventListener('dragstart'")&&appSource.includes("row.addEventListener('touchstart'")&&appSource.includes("{passive:false}")&&appSource.includes('touch.timer=setTimeout')&&appSource.includes('},360)'), 'desktop drag and Android-safe 360ms long-press drag must live in the canonical column-dialog owner without disabling ordinary vertical scroll');
assert.ok(!source.includes('MutationObserver'), 'TorrentFieldRegistry must not post-render repair the canonical column dialog');
console.log('Native column layout contract passed: exact qB source owns schema/defaults, user overrides survive, new columns auto-merge, future UNKNOWN fails closed, and desktop/mobile reorder is present.');
