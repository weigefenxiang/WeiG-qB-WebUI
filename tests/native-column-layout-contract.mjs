...
assert.ok(columnSource.includes("id:'column-configurator-dialog'")&&columnSource.includes('W.DialogRuntime.open(ensureDialog(),{draggable:true})'),'ColumnConfigurator must consume the shared DialogRuntime and expose one draggable desktop dialog.');assert.ok(appSource.includes('reorderable:false')&&uiSource.includes('reorderable:false')&&columnSource.includes('controls.hidden=!reorderable'),'source-ordered main/detail configurators must keep option positions stable and hide the contradictory up/down controls while table-header drag order remains independent.');
