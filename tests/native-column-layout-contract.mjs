...
assert.ok(tableCss.includes('width:min(var(--column-config-dialog-width,900px),calc(100vw - 24px))')&&tableCss.includes('[data-column-layout="2"] .shared-column-settings{grid-template-columns:repeat(2,minmax(0,1fr))}')&&tableCss.includes('[data-column-layout="3"] .shared-column-settings{grid-template-columns:repeat(3,minmax(0,1fr))}')&&tableCss.includes('font-size:var(--text-body)'),'ColumnConfigurator desktop layout must choose a compact two/three-column geometry with readable row text instead of fixed sparse columns.');
