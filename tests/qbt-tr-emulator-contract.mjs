...
result=emulateQbtDocument('<select id="locale_select">${LANGUAGE_OPTIONS}</select>',{catalog,behaviorEvidence:behavior,qbVersion:'5.2.3',locale:'zh_CN',materializeLanguageOptions:true});
...
result=emulateQbtDocument("const unresolved='\${LANGUAGE_OPTIONS}';",{catalog,behaviorEvidence:behavior,qbVersion:'5.2.3',locale:'zh_CN'});
assert.equal(result.text,"const unresolved='\${LANGUAGE_OPTIONS}';",'LANGUAGE_OPTIONS must remain inert outside the exact qB Preferences document materialization boundary');
assert.equal(result.mode,'none');
const serviceWorker=fs.readFileSync(new URL('../simulator/service-worker/service-worker.js',import.meta.url),'utf8');
assert.ok(serviceWorker.includes("const materializeLanguageOptions=path==='views/preferences.html'"),'Service Worker must scope qB LANGUAGE_OPTIONS materialization to the canonical Preferences document path');
assert.ok(serviceWorker.includes("materializeLanguageOptions&&text.includes('${LANGUAGE_OPTIONS}')"),'non-Preferences JavaScript/text containing the placeholder literal must not enter LANGUAGE_OPTIONS replacement');
...
console.log('QBT_TR emulator contract passed: Pages emulates exact translation behavior and path-scoped qB Preferences LANGUAGE_OPTIONS materialization without mutating unrelated JavaScript.');
