...
try{
  ...
  for(const key of ['export_dir','export_dir_fin']){const gate=page.locator('[data-projection-gate-for="'+key+'"] input[type="checkbox"]'),value=page.locator(preferenceRootSelector(key)+' input');assert(await gate.count()===1&&await value.count()===1,'Downloads: '+key+' presence gate/value missing');assert(await gate.isChecked()&&!await value.isDisabled(),'Downloads: '+key+' enabled projection state drifted');await gate.click();await page.waitForFunction(pref=>WeiG.SettingsState.draft[pref]==='',key);assert(await page.locator(preferenceRootSelector(key)+' input').isDisabled(),'Downloads: '+key+' value did not disable after gate off');await setPrefs(page,{[key]:key==='export_dir'?'/watch':'/watch/finished'},'downloads');}
