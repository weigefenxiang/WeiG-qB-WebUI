(function(global){
  ...
  var HANDOFF='weigg.loginHandshake.v2',LEGACY_HANDOFF='weigg.loginHandshake.v1',FAILURE='weigg.loginFailure.v1',PROBE='api/v2/app/preferences',HANDOFF_PARAM='__weigg_handoff',MAX_HANDOFF_AGE=120000;
  ...
  function clearHandoff(){try{sessionStorage.removeItem(HANDOFF);sessionStorage.removeItem(LEGACY_HANDOFF);}catch(_e){}}
  ...
  function navigatePublicFailure(){var url=publicEntryUrl();url.searchParams.set('__weigg_auth_failure','1');global.location.replace(url.href);}
  ...
  try{sessionStorage.removeItem(LEGACY_HANDOFF);}catch(_e){}
  W.SessionContract={probeEndpoint:PROBE,loginKind:loginKind,probeKind:probeKind,probe:probe,handoff:handoff,pendingHandoff:pendingHandoff,clearHandoff:clearHandoff,authenticatedEntryUrl:authenticatedEntryUrl,publicEntryUrl:publicEntryUrl,beginAuthenticatedHandoff:beginAuthenticatedHandoff,matchesAuthenticatedEntry:matchesAuthenticatedEntry,completeHandoff:completeHandoff,navigatePublicFailure:navigatePublicFailure,recordFailure:recordFailure,consumeFailure:consumeFailure};
})(window);
