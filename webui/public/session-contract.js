(function(global){
  ...
  var HANDOFF=K.loginHandshake||'weig.loginHandshake',FAILURE=K.loginFailure||'weig.loginFailure',LOGOUT_GUARD=K.logoutGuard||'weig.logoutGuard',LOCALE_INTENT=K.loginLocaleIntent||'weig.loginLocaleIntent',PROBE='api/v2/app/preferences',HANDOFF_PARAM=K.handoffParam||'__weig_handoff',AUTH_FAILURE_PARAM=K.authFailureParam||'__weig_auth_failure',MAX_HANDOFF_AGE=120000;
  ...
  function setLocaleIntent(locale){locale=String(locale||'').trim();if(!locale){clearLocaleIntent();return false;}return write(LOCALE_INTENT,{schemaVersion:1,locale:locale});}
  function consumeLocaleIntent(){var value=read(LOCALE_INTENT,true);return value&&value.schemaVersion===1?String(value.locale||'').trim():'';}
  function clearLocaleIntent(){try{sessionStorage.removeItem(LOCALE_INTENT);}catch(_e){}}
  ...
  W.SessionContract={probeEndpoint:PROBE,loginKind:loginKind,probeKind:probeKind,probe:probe,handoff:handoff,pendingHandoff:pendingHandoff,clearHandoff:clearHandoff,authenticatedEntryUrl:authenticatedEntryUrl,publicEntryUrl:publicEntryUrl,setLogoutGuard:setLogoutGuard,clearLogoutGuard:clearLogoutGuard,logoutGuarded:logoutGuarded,completePublicEntry:completePublicEntry,navigatePublic:navigatePublic,beginAuthenticatedHandoff:beginAuthenticatedHandoff,matchesAuthenticatedEntry:matchesAuthenticatedEntry,completeHandoff:completeHandoff,navigatePublicFailure:navigatePublicFailure,recordFailure:recordFailure,consumeFailure:consumeFailure,localeIntent:localeIntent,setLocaleIntent:setLocaleIntent,consumeLocaleIntent:consumeLocaleIntent,clearLocaleIntent:clearLocaleIntent,publicVersionIdentity:publicVersionIdentity};
})(window);
