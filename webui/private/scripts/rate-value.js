(function(global){
  ...
  function significant(value){var n=Number(value)||0;if(!n)return'0';var abs=Math.abs(n),power=Math.floor(Math.log10(abs)),decimals=Math.max(0,Math.min(6,2-power)),text=n.toFixed(decimals);return text.indexOf('.')>=0?text.replace(/0+$/,'').replace(/\.$/,''):text;}
