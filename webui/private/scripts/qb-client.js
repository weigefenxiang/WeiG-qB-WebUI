(function(global){
  ...
  function sourceActionDescriptor(action){var R=W.ReleaseProfile,profile=R&&R.current&&R.current();if(!R||!profile)return undefined;if(profile.fallback===true)return null;if(typeof R.actionDescriptor==='function')return R.actionDescriptor(action);if(typeof R.hasAction==='function'&&R.hasAction(action))return{sourceAction:action,parameters:[],required:[],optional:[]};return null;}
  function requireSourceAction(action,path){var desc=sourceActionDescriptor(action);if(desc===null)throw new ApiError(localeText('This API action is not source-proven for the current qBittorrent release.','当前 qBittorrent 版本未由源码证明支持此 API 操作。'),400,path);return desc;}
  ...
  Client.prototype.detect=async function(){var results=await Promise.all([this.request('app/version',{type:'text'}),this.request('app/webapiVersion',{type:'text'}).catch(function(){return '2.0';})]);this.qbVersion=(results[0]||'0').trim();this.webApiVersion=(results[1]||'0').trim();this.major=versionParts(this.qbVersion)[0]||0;var v=this.webApiVersion;this.capabilities={legacy4:this.major===4,modern5:this.major>=5,certified:false,rss:atLeast(v,'2.1.0'),search:atLeast(v,'2.1.1'),trackerEdit:atLeast(v,'2.2.0'),rssRefresh:atLeast(v,'2.2.1'),tags:atLeast(v,'2.3.0'),peerBan:atLeast(v,'2.3.0'),buildInfo:atLeast(v,'2.3.0'),renameFile:atLeast(v,'2.4.0'),stalledFilter:atLeast(v,'2.4.1'),contentPath:atLeast(v,'2.6.1'),addTags:atLeast(v,'2.6.2'),renameFolder:atLeast(v,'2.8.0'),fileIndexes:atLeast(v,'2.8.2'),tagFilter:atLeast(v,'2.8.3'),privateFlag:this.major>=5,logs:atLeast(this.qbVersion,'4.1.0'),globalSpeedLimits:true,altSpeedLimits:true,mainData:true,sessionStats:true,torrentCreator:this.major>=5&&atLeast(v,'2.11.0'),cookies:atLeast(v,'2.11.3'),trackerEditUrl:atLeast(v,'2.13.0'),structuredTorrentAdd:atLeast(v,'2.14.0'),webseeds:true,categories:true,preferences:true};if(this.qbVersion.replace(/^v/i,'')==='4.3.3')this.capabilities.renameFolder=true;return this;};
  Client.prototype.applyCapabilityRegistry=function(registry){var R=W.ReleaseProfile,self=this,map={privateFilter:'privateFlag',tags:'tags',stalledFilter:'stalledFilter',rss:'rss',search:'search',logs:'logs',peerBan:'peerBan'};this.capabilities.certified=!!(R&&R.isCertified&&R.isCertified());if(registry&&typeof registry.supports==='function')Object.keys(map).forEach(function(id){self.capabilities[map[id]]=!!registry.supports(id);});return this;};
  Client.prototype.getTorrents=function(opts){opts=opts||{};var q=new URLSearchParams(),self=this,R=W.ReleaseProfile,next=Object.assign({},opts);if(next.filter!==undefined&&next.filter!==null&&next.filter!==''){var mapped=R&&R.upstreamTorrentFilter?R.upstreamTorrentFilter(next.filter):null;if(mapped!==null)next.filter=mapped;else if(R&&R.upstreamTorrentFilter)return Promise.reject(new ApiError(localeText('This torrent filter is not supported by the current qBittorrent release.','当前 qBittorrent 版本不支持此 Torrent 筛选。'),400,'torrents/info'));else if(next.filter==='paused'&&self.major>=5)next.filter='stopped';else if(next.filter==='stopped'&&self.major===4)next.filter='paused';}['filter','category','tag','sort','reverse','limit','offset','hashes'].forEach(function(k){if(next[k]===undefined||next[k]===null||next[k]==='')return;q.set(k,String(next[k]));});return this.request('torrents/info?'+q.toString());};
  ...
  Client.prototype.getPreferences=async function(){requireSourceAction('appcontroller.h:preferencesAction','app/preferences');return this.request('app/preferences');};
  ...
  Client.prototype.properties=function(hash){requireSourceAction('torrentscontroller.h:propertiesAction','torrents/properties');return this.request('torrents/properties?hash='+encodeURIComponent(hash));};
  Client.prototype.files=function(hash){requireSourceAction('torrentscontroller.h:filesAction','torrents/files');return this.request('torrents/files?hash='+encodeURIComponent(hash));};
  Client.prototype.setFilePriority=function(hash,ids,priority){requireSourceAction('torrentscontroller.h:filePrioAction','torrents/filePrio');return this.request('torrents/filePrio',{method:'POST',form:{hash:hash,id:ids,priority:priority},type:'void'});};
  Client.prototype.trackers=function(hash){requireSourceAction('torrentscontroller.h:trackersAction','torrents/trackers');return this.request('torrents/trackers?hash='+encodeURIComponent(hash));};
  Client.prototype.addTrackers=function(hash,urls){requireSourceAction('torrentscontroller.h:addTrackersAction','torrents/addTrackers');return this.request('torrents/addTrackers',{method:'POST',form:{hash:hash,urls:urls},type:'void'});};
  ...
  Client.prototype.editTracker=async function(hash,origUrl,newUrl){var desc=requireSourceAction('torrentscontroller.h:editTrackerAction','torrents/editTracker'),form={};if(desc!==undefined){var parameters=Array.isArray(desc.parameters)?desc.parameters:[],required=Array.isArray(desc.required)?desc.required:[];if(parameters.indexOf('hash')<0)throw new ApiError(localeText('The current editTracker source contract does not expose the hash parameter.','当前 editTracker 源码契约未声明 hash 参数。'),400,'torrents/editTracker');form.hash=hash;if(parameters.indexOf('origUrl')>=0)form.origUrl=origUrl;else if(parameters.indexOf('url')>=0)form.url=origUrl;else throw new ApiError(localeText('The current editTracker source contract does not expose a supported tracker URL parameter.','当前 editTracker 源码契约未声明可支持的 Tracker URL 参数。'),400,'torrents/editTracker');if(parameters.indexOf('newUrl')>=0)form.newUrl=newUrl;else throw new ApiError(localeText('The current editTracker source contract cannot prove URL editing.','当前 editTracker 源码契约无法证明支持 URL 编辑。'),400,'torrents/editTracker');for(var i=0;i<required.length;i++)if(!Object.prototype.hasOwnProperty.call(form,required[i]))throw new ApiError(localeText('The current editTracker source contract requires a parameter this client cannot provide.','当前 editTracker 源码契约要求客户端无法安全提供的参数。'),400,'torrents/editTracker');return this.request('torrents/'+(desc.endpoint||'editTracker'),{method:'POST',form:form,type:'void'});}form={hash:hash,newUrl:newUrl};if(this.capabilities.trackerEditUrl===true||atLeast(this.webApiVersion,'2.13.0'))form.url=origUrl;else form.origUrl=origUrl;return this.request('torrents/editTracker',{method:'POST',form:form,type:'void'});};
  Client.prototype.webseeds=function(hash){requireSourceAction('torrentscontroller.h:webseedsAction','torrents/webseeds');return this.request('torrents/webseeds?hash='+encodeURIComponent(hash));};
  Client.prototype.peers=async function(hash){requireSourceAction('synccontroller.h:torrentPeersAction','sync/torrentPeers');var data=await this.request('sync/torrentPeers?rid=0&hash='+encodeURIComponent(hash));if(!data||typeof data!=='object'||Array.isArray(data)||!data.peers||typeof data.peers!=='object'||Array.isArray(data.peers))throw new ApiError(localeText('The Torrent Peers API returned an unexpected response.','Torrent Peers API 返回了异常响应。'),200,'sync/torrentPeers');var peers=data.peers;return Object.keys(peers).map(function(k){var p=peers[k]||{};p.__key=k;return p;});};
  Client.prototype.banPeers=function(peers){requireSourceAction('transfercontroller.h:banPeersAction','transfer/banPeers');return this.request('transfer/banPeers',{method:'POST',form:{peers:peers},type:'void'});};
  ...
  Client.prototype.logs=async function(lastId){requireSourceAction('logcontroller.h:mainAction','log/main');var q='normal=true&info=true&warning=true&critical=true&last_known_id='+(lastId==null?-1:lastId);return normalizeLogItems(await this.request('log/main?'+q));};
  Client.prototype.peerLogs=async function(lastId){requireSourceAction('logcontroller.h:peersAction','log/peers');return normalizeLogItems(await this.request('log/peers?last_known_id='+(lastId==null?-1:lastId)));};
  Client.prototype.rssItems=function(withData){requireSourceAction('rsscontroller.h:itemsAction','rss/items');return this.request('rss/items?withData='+(withData?'true':'false'));};
  Client.prototype.rssAddFeed=function(url,path){requireSourceAction('rsscontroller.h:addFeedAction','rss/addFeed');return this.request('rss/addFeed',{method:'POST',form:{url:url,path:path||''},type:'void'});};
  Client.prototype.rssRemoveItem=function(path){requireSourceAction('rsscontroller.h:removeItemAction','rss/removeItem');return this.request('rss/removeItem',{method:'POST',form:{path:path},type:'void'});};
  Client.prototype.rssRefreshItem=function(itemPath){requireSourceAction('rsscontroller.h:refreshItemAction','rss/refreshItem');return this.request('rss/refreshItem',{method:'POST',form:{itemPath:itemPath},type:'void'});};
  Client.prototype.rssRules=function(){requireSourceAction('rsscontroller.h:rulesAction','rss/rules');return this.request('rss/rules');};
  Client.prototype.searchPlugins=function(){requireSourceAction('searchcontroller.h:pluginsAction','search/plugins');return this.request('search/plugins');};
  Client.prototype.searchStart=function(pattern,plugins,category){requireSourceAction('searchcontroller.h:startAction','search/start');return this.request('search/start',{method:'POST',form:{pattern:pattern,plugins:plugins||'enabled',category:category||'all'}});};
  Client.prototype.searchStatus=function(id){requireSourceAction('searchcontroller.h:statusAction','search/status');return this.request('search/status'+(id!=null?'?id='+encodeURIComponent(id):''));};
  Client.prototype.searchResults=function(id,limit,offset){requireSourceAction('searchcontroller.h:resultsAction','search/results');return this.request('search/results?id='+encodeURIComponent(id)+'&limit='+(limit||50)+'&offset='+(offset||0));};
  Client.prototype.searchStop=function(id){requireSourceAction('searchcontroller.h:stopAction','search/stop');return this.request('search/stop',{method:'POST',form:{id:id},type:'void'});};
