(function(global){
  ...
  function renderWorkspace(){var root=workspaceRoot();if(!root)return;root.classList.add('rss-workspace');root.dataset.rssState=workspace.status;root.textContent='';
    var feedsPane=document.createElement('section');feedsPane.className='rss-workspace__pane rss-workspace__feeds';var feedsHead=document.createElement('header');feedsHead.className='rss-workspace__pane-head';var feedsTitle=document.createElement('strong');feedsTitle.textContent=text('RSS subscriptions','RSS 订阅');var feedsActions=document.createElement('span');feedsActions.className='rss-workspace__pane-actions';feedsHead.append(feedsTitle,feedsActions);var feedList=document.createElement('div');feedList.className='rss-workspace__list rss-feed-list';feedsPane.append(feedsHead,feedList);
    var articlesPane=document.createElement('section');articlesPane.className='rss-workspace__pane rss-workspace__articles';var articlesHead=document.createElement('header');articlesHead.className='rss-workspace__pane-head';var articlesTitle=document.createElement('strong');articlesTitle.textContent=text('Articles','文章');var articleActions=document.createElement('span');articleActions.className='rss-workspace__pane-actions';if(hasAction('rsscontroller.h:rulesAction')){var downloader=workspaceAction('RSS Downloader',function(){});if(W.ActionRegistry)W.ActionRegistry.bind(downloader,'rss.downloader.open');else downloader.disabled=true;articleActions.appendChild(downloader);}articlesHead.append(articlesTitle,articleActions);var articleList=document.createElement('div');articleList.className='rss-workspace__list rss-article-list';articlesPane.append(articlesHead,articleList);root.append(feedsPane,articlesPane);
    if(workspace.status==='LOADING'){workspaceEmpty(feedList,text('Loading RSS subscriptions…','正在加载 RSS 订阅…'));workspaceEmpty(articleList,text('Loading articles…','正在加载文章…'));return;}
    if(workspace.status==='UNSUPPORTED'){workspaceEmpty(feedList,text('This qBittorrent release does not expose the RSS items API.','当前 qBittorrent 版本未提供 RSS items API。'));workspaceEmpty(articleList,text('RSS browsing is unavailable for this runtime.','当前运行环境无法浏览 RSS。'));return;}
    if(workspace.status==='ERROR'){workspaceEmpty(feedList,text('RSS could not be loaded.','RSS 无法加载。'));workspaceEmpty(articleList,workspace.error||text('RSS request failed.','RSS 请求失败。'));return;}
    if(!workspace.feeds.length){workspaceEmpty(feedList,text('No RSS subscriptions yet.','还没有 RSS 订阅。'));workspaceEmpty(articleList,text('Add a feed to begin browsing articles.','添加 Feed 后即可浏览文章。'));return;}
    ...
    if(canWrite('rsscontroller.h:refreshItemAction'))feedsActions.appendChild(workspaceAction(text('Refresh selected','刷新所选'),async function(){var client=currentClient();if(!client)return;await client.rssRefreshItem(selected.path);await loadWorkspace();}));
    if(canWrite('rsscontroller.h:markAsReadAction')&&selectedUnread>0)feedsActions.appendChild(workspaceAction(text('Mark read','全部已读'),async function(){var client=currentClient();if(!client)return;await client.rssMarkAsRead(selected.path);await loadWorkspace();}));
    if(canWrite('rsscontroller.h:removeItemAction'))feedsActions.appendChild(workspaceAction(text('Remove','删除'),async function(){if(!global.confirm(text('Remove this RSS subscription?','删除这个 RSS 订阅？')))return;var client=currentClient();if(!client)return;await client.rssRemoveItem(selected.path);workspace.selectedPath='';await loadWorkspace();},'btn--danger-ghost'));
    articlesTitle.textContent=String(selected.data&&selected.data.title||selected.name||text('Articles','文章'));
    if(!selectedArticles.length){workspaceEmpty(articleList,text('This feed has no articles.','这个 Feed 暂无文章。'));return;}
    selectedArticles.forEach(function(article){article=article&&typeof article==='object'?article:{};var row=document.createElement('article');row.className='rss-article-row tool-row';row.dataset.read=article.isRead===true?'true':'false';var copy=document.createElement('span');copy.className='rss-article-row__copy';var title;if(article.link){title=document.createElement('a');title.href=String(article.link);title.target='_blank';title.rel='noopener noreferrer';title.textContent=String(article.title||article.id||text('Untitled article','无标题文章'));}else{title=document.createElement('strong');title.textContent=String(article.title||article.id||text('Untitled article','无标题文章'));}var meta=document.createElement('small');meta.textContent=[article.author,article.date].filter(Boolean).join(' · ');copy.append(title,meta);var actions=document.createElement('span');actions.className='rss-article-row__actions';var read=document.createElement('span');read.className='rss-article-row__state';read.textContent=article.isRead===true?text('Read','已读'):text('Unread','未读');actions.appendChild(read);if(canWrite('rsscontroller.h:markAsReadAction')&&article.isRead!==true&&article.id!=null)actions.appendChild(workspaceAction(text('Mark read','标记已读'),async function(){var client=currentClient();if(!client)return;await client.rssMarkAsRead(selected.path,article.id);await loadWorkspace();}));row.append(copy,actions);articleList.appendChild(row);});
    if(W.RSS&&typeof W.RSS.apply==='function')W.RSS.apply(root);
  }
  async function loadWorkspace(){var client=currentClient();if(!hasAction('rsscontroller.h:itemsAction')){workspace.status='UNSUPPORTED';workspace.feeds=[];workspace.error='';renderWorkspace();return false;}if(!client){workspace.status='ERROR';workspace.error=text('qBittorrent client is not ready.','qBittorrent 客户端尚未就绪。');renderWorkspace();return false;}workspace.status='LOADING';workspace.error='';renderWorkspace();try{var data=await client.rssItems(true),feeds=collectWorkspaceFeeds(data||{},'',[]),selected=workspace.selectedPath;workspace.feeds=feeds;if(!selected||!feeds.some(function(feed){return feed.path===selected;}))workspace.selectedPath=feeds[0]&&feeds[0].path||'';workspace.status=feeds.length?'READY':'EMPTY';renderWorkspace();return true;}catch(error){workspace.status='ERROR';workspace.error=String(error&&error.message||error);workspace.feeds=[];renderWorkspace();return false;}}
  async function addWorkspaceFeed(url,path){var client=currentClient();if(!client)throw new Error(text('qBittorrent client is not ready.','qBittorrent 客户端尚未就绪。'));if(!canWrite('rsscontroller.h:addFeedAction'))throw new Error(text('Adding RSS feeds is not source-proven for this qBittorrent release.','当前 qBittorrent 版本没有源码证明支持添加 RSS Feed。'));await client.rssAddFeed(String(url||''),String(path||''));return loadWorkspace();}
  ...
  function renderList(){
    ...
    var add=button('+','btn--ghost');add.classList.add('rss-rule-collection__action');add.dataset.rssRuleCollectionAction='add';add.setAttribute('aria-label',text('Add new rule','新增规则'));add.title=text('Add new rule','新增规则');add.disabled=!collectionAvailable()||!canWrite('rsscontroller.h:setRuleAction');add.addEventListener('click',startNewRule);
    var remove=button('−','btn--ghost');remove.classList.add('rss-rule-collection__action');remove.dataset.rssRuleCollectionAction='remove';remove.setAttribute('aria-label',text('Delete rule','删除规则'));remove.title=text('Delete rule','删除规则');remove.disabled=!state.newDraft&&(!state.selected||!collectionAvailable()||!canWrite('rsscontroller.h:removeRuleAction'));remove.addEventListener('click',removeRule);actions.append(remove,add);head.append(title,actions);state.list.appendChild(head);
    ...
    if(state.newDraft){var draftRow=button(text('New rule','新规则'),'btn--ghost');draftRow.classList.add('is-active');draftRow.dataset.rssRuleDraft='new';draftRow.disabled=true;list.appendChild(draftRow);}
    ...
    if(!names.length&&!state.newDraft){var empty=document.createElement('p');empty.className='text-description';empty.textContent=text('No download rules.','暂无下载规则。');list.appendChild(empty);}state.list.appendChild(list);
  }
  ...
  function renderEditor(){
    ...
    var form=document.createElement('div');form.className='settings-grid';var nameControl=input('text',state.newDraft?'':state.selected);nameControl.placeholder=text('New rule name','新规则名称');nameControl.dataset.rssRuleField='name';form.appendChild(field(text('Rule name','规则名称'),nameControl));state.controls.name=nameControl;
  ...
  async function install(){
    ...
    var actions=document.createElement('div');actions.className='inline-form';var refresh=button(text('Refresh rules','刷新规则'),'btn--ghost');refresh.addEventListener('click',loadRules);var close=button(text('Close','关闭'),'btn--ghost');close.addEventListener('click',function(){W.DialogRuntime.close(dialog);});actions.append(refresh,close);head.appendChild(actions);panel.appendChild(head);
  ...
  async function openResolved(){if(W.Router&&W.Router.go)W.Router.go('rss');else location.hash='#/rss';await install();renderAutoDownloadState();if(!state.dialog)throw new Error(text('RSS Downloader is unavailable for this qBittorrent runtime.','当前 qBittorrent 运行环境不支持 RSS Downloader。'));W.DialogRuntime.open(state.dialog,{draggable:true});return true;}
  async function open(){return W.ActionRegistry?W.ActionRegistry.invoke('rss.downloader.open'):false;}
  function registerDownloaderAction(){if(!W.ActionRegistry)return;W.ActionRegistry.register('rss.downloader.open',{resolve:async function(){await ensureManifest();if(state.manifestStatus==='ERROR')return{state:'ERROR',reason:text('RSS Downloader compatibility facts could not be loaded.','RSS Downloader 兼容性事实加载失败。'),error:state.manifestError};if(!state.manifest||!hasAction('rsscontroller.h:rulesAction'))return{state:'UNSUPPORTED',reason:text('RSS Downloader is not source-proven for this qBittorrent runtime.','当前 qBittorrent 运行环境没有源码证明支持 RSS Downloader。')};return{state:'AVAILABLE'};},run:openResolved});}
