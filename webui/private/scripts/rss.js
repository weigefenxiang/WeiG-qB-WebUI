(function(global){
  ...
  var state={rules:{},selected:'',draft:null,newDraft:false,matching:{},busy:false,root:null,dialog:null,list:null,editor:null,status:null,disabledNote:null,matchingRoot:null,manifest:null,manifestTask:null,manifestStatus:'IDLE',manifestError:null,controls:{},returnHash:''};
  ...
  function captureReturnHash(){var route=W.Router&&W.Router.route?W.Router.route():null;return route&&route.name!=='rss'?String(location.hash||'#/'):'';}
  function restoreReturnHash(){var target=state.returnHash;state.returnHash='';if(!target||String(location.hash||'')===target)return;if(target.charAt(0)==='#')location.hash=target;}
  function teardown(){state.returnHash='';if(state.dialog&&W.DialogRuntime)W.DialogRuntime.remove(state.dialog);else if(state.root)state.root.remove();state.root=null;state.dialog=null;state.list=null;state.editor=null;state.status=null;state.disabledNote=null;state.matchingRoot=null;state.newDraft=false;state.controls={};}
  async function install(){
    ...
    var dialog=W.DialogRuntime.create({id:'rss-rules-dialog',className:'dialog surface surface--modal rss-rules-dialog',draggable:true});dialog.dataset.dialogMobile='compact';if(dialog.dataset.rssReturnBound!=='1'){dialog.dataset.rssReturnBound='1';dialog.addEventListener('close',restoreReturnHash);}var panel=document.createElement('div');panel.id='rss-rules-panel';panel.className='rss-downloader-workspace';var head=document.createElement('div');head.className='workspace__header';var title=document.createElement('h2');title.textContent='RSS Downloader';head.appendChild(title);
  ...
  async function openResolved(){state.returnHash=captureReturnHash();if(W.Router&&W.Router.go)W.Router.go('rss');else location.hash='#/rss';await install();renderAutoDownloadState();if(!state.dialog)throw new Error(tr('rss.rule.downloaderUnavailable'));W.DialogRuntime.open(state.dialog,{draggable:true});return true;}
