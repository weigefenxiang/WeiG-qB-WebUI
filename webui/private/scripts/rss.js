(function(global){
  ...
  function receipt(subject,label,value){if(W.Feedback&&W.Feedback.receipt)W.Feedback.receipt({subject:String(subject||'RSS'),results:[{label:String(label||''),value:value==null?'':String(value)}]},'success');}
  ...
  async function addWorkspaceFeed(url,path){var client=currentClient();if(!client)throw new Error(tr('runtime.clientNotReady'));if(!canWrite('rsscontroller.h:addFeedAction'))throw new Error(tr('rss.workspace.addFeedUnproven'));await client.rssAddFeed(String(url||''),String(path||''));var loaded=await loadWorkspace();receipt('RSS',tr('rss.workspace.feed'),String(url||''));return loaded;}
  ...
  async function saveRule(){
    ...
    try{if(state.selected&&state.selected!==value.name){if(!canWrite('rsscontroller.h:renameRuleAction'))throw new Error(tr('rss.rule.renameUnproven'));await client.rssRenameRule(state.selected,value.name);}await client.rssSetRule(value.name,value.rule);state.selected=value.name;state.newDraft=false;await loadRules();await loadMatchingArticles();setStatus(tr('rss.rule.saved'));receipt('RSS Downloader',nativeCopy('rules','Download Rules'),value.name);}catch(error){setStatus(tr('rss.rule.saveFailedPrefix')+(error&&error.message||error),true);}finally{state.busy=false;}
  }
  async function removeRule(){if(state.busy)return;if(state.newDraft&&!state.selected){state.newDraft=false;state.draft=null;state.matching={};render();setStatus(tr('rss.rule.draftDiscarded'));return;}if(!state.selected)return;var client=currentClient();if(!client||!state.manifest||!collectionAvailable()||!canWrite('rsscontroller.h:removeRuleAction'))return;state.busy=true;setStatus(tr('rss.rule.removing'));try{var removedName=state.selected;await client.rssRemoveRule(state.selected);state.selected='';state.draft=null;state.newDraft=false;state.matching={};await loadRules();setStatus(tr('rss.rule.removed'));receipt('RSS Downloader',tr('rss.rule.delete'),removedName);}catch(error){setStatus(tr('rss.rule.removeFailedPrefix')+(error&&error.message||error),true);}finally{state.busy=false;}}
  ...
  async function install(){
    ...
    var dialog=W.DialogRuntime.create({id:'rss-rules-dialog',className:'dialog surface surface--modal rss-rules-dialog',draggable:true});dialog.dataset.dialogMobile='compact';var panel=document.createElement('div');panel.id='rss-rules-panel';panel.className='rss-downloader-workspace';var head=document.createElement('div');head.className='workspace__header';var title=document.createElement('h2');title.textContent='RSS Downloader';head.appendChild(title);
    ...
    var layout=document.createElement('div');layout.className='settings-layout';state.list=document.createElement('nav');state.list.id='rss-rules-list';state.list.className='settings-tabs';state.editor=document.createElement('div');state.editor.id='rss-rule-editor';state.editor.className='settings-content';layout.appendChild(state.list);layout.appendChild(state.editor);panel.appendChild(layout);state.status=document.createElement('p');state.status.className='text-description';panel.appendChild(state.status);var dialogBody=document.createElement('div');dialogBody.className='dialog__body rss-rules-dialog__body';dialogBody.appendChild(panel);dialog.appendChild(dialogBody);state.root=panel;state.dialog=dialog;renderAutoDownloadState();await loadRules();
  }
  async function openResolved(){await install();renderAutoDownloadState();if(!state.dialog)throw new Error(tr('rss.rule.downloaderUnavailable'));await loadRules();W.DialogRuntime.open(state.dialog,{draggable:true});return true;}
