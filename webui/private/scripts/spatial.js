(function(global){
  ...
  function init(){installFacetControls();if(W.I18n&&W.I18n.loadQbOwnedText)W.I18n.loadQbOwnedText().then(sync).catch(function(error){console.error('[WeiG qB copy]',error);});capabilitiesReady().then(function(registry){return registry.load().then(function(){registry.sync();return registry;});}).catch(function(error){console.error('[WeiG capabilities]',error);});}
