(function(global){
  ...
  var VERSION='0.3.7',cssObserver=null,ordering=false,legacySelectObserver=null,filterIntentTimer=null;
  ...
  function dedupeLegacyMobileFilter(){
    ...
    var explicit=host.querySelector(':scope > .ui-select:not(.ui-select--native)');
    if(C&&C.__floatingSelectV036&&typeof C.selectControl==='function'&&(!explicit||!explicit.__uiMenu)){
      if(explicit)explicit.remove();
      explicit=C.selectControl({options:mobileFilterOptions(),value:mobileFilterValue(),ariaLabel:(native&&native.getAttribute('aria-label'))||'Torrent filter',onChange:function(value){var target=document.querySelector('#filter-nav [data-filter="'+value+'"]');if(target)target.click();}});
      host.appendChild(explicit);
    }
    if(explicit&&typeof explicit.setOptions==='function'&&typeof explicit.setValue==='function'){explicit.setOptions(mobileFilterOptions());explicit.setValue(mobileFilterValue());}
  }
  function ownLegacyMobileFilter(){dedupeLegacyMobileFilter();if(legacySelectObserver||!document.documentElement)return;legacySelectObserver=new MutationObserver(function(){requestAnimationFrame(dedupeLegacyMobileFilter);});legacySelectObserver.observe(document.documentElement,{childList:true,subtree:true});}
  function visibleTorrentSignature(){return Array.from(document.querySelectorAll('.torrent-mobile-card,.torrent-row')).slice(0,6).map(function(row){return row.dataset.hash||'';}).join('|');}
  function filterResultSettled(beforeCount,beforeSignature){var countNode=document.getElementById('torrent-count'),count=Number(countNode&&countNode.textContent),rows=document.querySelectorAll('.torrent-mobile-card,.torrent-row').length,signature=visibleTorrentSignature();if(!Number.isFinite(count)||rows>count)return false;return String(countNode.textContent)!==String(beforeCount)||signature!==beforeSignature;}
  function installFilterIntentGuard(){
    var nav=document.getElementById('filter-nav');if(!nav||nav.dataset.v037IntentGuard==='1')return;nav.dataset.v037IntentGuard='1';
    nav.addEventListener('click',function(event){var button=event.target&&event.target.closest&&event.target.closest('[data-filter]');if(!button)return;var wanted=button.dataset.filter||'all',beforeCount=document.getElementById('torrent-count')?.textContent||'',beforeSignature=visibleTorrentSignature(),tries=0,started=Date.now();clearTimeout(filterIntentTimer);
      function verify(){if(mobileFilterValue()!==wanted)return;if(filterResultSettled(beforeCount,beforeSignature))return;var loading=document.getElementById('list-loading'),busy=loading&&!loading.classList.contains('is-hidden');if((busy||Date.now()-started<120)&&Date.now()-started<2200){filterIntentTimer=setTimeout(verify,90);return;}if(tries++>=2)return;var refresh=document.getElementById('refresh-btn');if(refresh)refresh.click();filterIntentTimer=setTimeout(verify,220);}
      filterIntentTimer=setTimeout(verify,120);
    });
  }
  ...
  function start(){
    css();document.documentElement.dataset.v037='1';ownLegacyMobileFilter();installFilterIntentGuard();
    load('scripts/i18n-v037.js','i18n-037')
      ...
      .then(function(){
        var init=function(){ensureCssOrder();dedupeLegacyMobileFilter();installFilterIntentGuard();if(W.V037Settings)W.V037Settings.init();if(W.V037Selection)W.V037Selection.init();if(W.V037Layout)W.V037Layout.init();if(W.V037UiSystem)W.V037UiSystem.init();dedupeLegacyMobileFilter();installFilterIntentGuard();W.V037={version:VERSION,settings:W.V037Settings,selection:W.V037Selection,layout:W.V037Layout,ui:W.V037UiSystem};global.dispatchEvent(new CustomEvent('weigg:v037ready'));};
