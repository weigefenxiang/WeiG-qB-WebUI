(function(global){
  ...
  function ensureDialog(){
    ...
    eyebrow.className='eyebrow';eyebrow.dataset.actionEditorEyebrow='1';title.dataset.actionEditorTitle='1';copy.append(eyebrow,title);close.type='button';close.className='icon-btn';close.textContent='×';close.dataset.actionEditorClose='1';close.onclick=function(){W.DialogRuntime.close(dialog);};head.append(copy,close);
    var body=document.createElement('div');body.className='dialog__body torrent-action-editor__body';
    var actions=document.createElement('div');actions.className='dialog__actions torrent-action-editor__actions';var left=document.createElement('span');left.className='torrent-action-editor__footer-left';left.dataset.actionEditorUnitSlot='1';var cancel=document.createElement('button'),confirm=document.createElement('button');cancel.type='button';cancel.className='btn btn--ghost';cancel.dataset.actionEditorCancel='1';cancel.onclick=function(){W.DialogRuntime.close(dialog);};confirm.type='button';confirm.className='btn btn--primary';confirm.dataset.actionEditorConfirm='1';actions.append(left,cancel,confirm);
    dialog.append(head,body,actions);
    dialog.addEventListener('close',function(){active=null;unitControl=null;});
    global.addEventListener('weig:languagechange',function(){if(dialog&&dialog.isConnected)paintChrome();});
    return dialog;
  }
  function paintChrome(){if(!dialog)return;var eyebrow=dialog.querySelector('[data-action-editor-eyebrow]'),close=dialog.querySelector('[data-action-editor-close]'),cancel=dialog.querySelector('[data-action-editor-cancel]'),confirm=dialog.querySelector('[data-action-editor-confirm]'),title=dialog.querySelector('[data-action-editor-title]');if(eyebrow)eyebrow.textContent=tr('dialog.action.eyebrow');if(close)close.setAttribute('aria-label',tr('app.close'));if(cancel)cancel.textContent=tr('app.cancel');if(confirm)confirm.textContent=tr('app.confirm');if(title&&active&&active.definition)title.textContent=tr(active.definition.key||'dialog.action.title');}
  function resetFrame(titleKey){
    ...
    paintChrome();title.textContent=tr(titleKey||'dialog.action.title');body.textContent='';slot.textContent='';unitControl=null;return{dialog:d,title:title,body:body,slot:slot,confirm:d.querySelector('[data-action-editor-confirm]')};
  }
