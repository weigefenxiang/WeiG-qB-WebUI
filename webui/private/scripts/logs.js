(function(global){
  ...
  function renderRows(reset,added){var root=U.$('logs-content');if(!root)return;if(!root.querySelector('[data-weigg-log-shell]'))buildShell(root);var list=root.querySelector('.logs-list'),empty=root.querySelector('.logs-empty'),status=root.querySelector('.logs-status');if(!list)return;var items=filtered(),previousTop=list.scrollTop;if(!state.virtual||state.virtual.el!==list){state.virtual=new W.VirtualList(list,{rowHeight:54,overscan:8,renderRow:function(x){var row=document.createElement('div');row.className='logs-row';row.dataset.tone=typeTone(Number(x.type));row.setAttribute('role','listitem');row.dataset.logId=String(itemId(x));var msg=document.createElement('span');msg.className='logs-message';msg.textContent=x.message||'';var ts=document.createElement('time');ts.className='logs-time';ts.dateTime=timestampMs(x)?new Date(timestampMs(x)).toISOString():'';ts.textContent=formatTime(x);var level=document.createElement('span');level.className='logs-level status-pill';level.dataset.tone=typeTone(Number(x.type));level.textContent=typeLabel(Number(x.type));row.append(msg,ts,level);return row;}});}state.virtual.setItems(items);if(reset&&state.virtual.resetScroll)state.virtual.resetScroll();else if(!state.follow&&added>0&&previousTop>0){var keep=previousTop+added*54;list.__weiggVirtualScrollTop=keep;list.scrollTop=keep;state.virtual.render();}empty.classList.toggle('is-hidden',items.length!==0);status.textContent=tr('v036.logs.showing',{shown:items.length,total:state.items.length},'Showing '+items.length+' / '+state.items.length+' · newest first');if(state.follow)scrollLatest();}
