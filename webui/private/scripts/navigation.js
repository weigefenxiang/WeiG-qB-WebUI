(function(global){
  ...
  var KEY='weigg.torrentListContext',routeModules={};
  ...
  function loadRouteModule(name){if(name!=='rss'||W.RSSRules||routeModules[name])return;routeModules[name]=true;var script=document.createElement('script');script.src=W.buildAssetUrl?W.buildAssetUrl('scripts/rss.js'):'scripts/rss.js';script.async=true;script.dataset.weiggRouteModule=name;script.addEventListener('error',function(){routeModules[name]=false;console.error('[WeiG route module] failed to load '+name);},{once:true});document.head.appendChild(script);}
