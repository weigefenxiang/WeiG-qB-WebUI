(function(global){
  ...
  function loadRouteModule(name){if(name!=='rss'||W.RSSRules||routeModules[name])return;routeModules[name]=true;var script=document.createElement('script');script.src=W.buildAssetUrl?W.buildAssetUrl('scripts/rss.js'):'scripts/rss.js';script.async=true;script.dataset.weigRouteModule=name;script.addEventListener('error',function(){routeModules[name]=false;console.error('[WeiG route module] failed to load '+name);},{once:true});document.head.appendChild(script);}
  function sync(){var tabs=document.querySelector('#detail-view .detail-tabs'),slot=document.getElementById('detail-context-slot'),r=route(),b=document.querySelector('[data-detail-back]');loadRouteModule(r.name);if(r.name!=='torrent'){if(b)b.remove();restore();return;}if(!tabs)return;if(!b)b=createBack();var mobile=isMobile(),target=mobile&&slot?slot:tabs;if(target&&b.parentElement!==target){if(target===tabs)tabs.insertBefore(b,tabs.firstChild);else target.appendChild(b);}b.classList.toggle('detail-context-back--mobile',mobile);var label=b.querySelector('span');if(label){var I=W.I18n;label.textContent=I&&I.t?I.t(mobile?'detail.backShort':'detail.back'):(mobile?'Back':'Back to torrents');}}
  ...
  W.Navigation={goHome:function(){if(W.Router)W.Router.home();else location.hash='#/';},backFromDetail:back,restoreListContext:restore,syncDetailBack:sync,loadRouteModule:loadRouteModule};
