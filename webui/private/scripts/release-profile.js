(function(global){
  ...
  function inherited(qb,webApi){var target=version(qb),api=version(webApi),candidates=(catalog||[]).filter(function(item){return item&&item.officialWeiGSupport!==false&&sameSeries(item.qbVersion,target)&&compare(item.qbVersion,target)<=0;}).sort(function(a,b){return compare(b.qbVersion,a.qbVersion);});var base=candidates[0]||null;if(!base)return null;if(api&&api!=='0'&&base.webApiVersion&&compare(api,base.webApiVersion)<0)return null;return resolution(base,qb,'INHERITED');}
  async function bind(client){await load();var qb=client&&client.qbVersion||'0',api=client&&client.webApiVersion||'0',hit=exact(qb);if(hit){var mode=rawVersion(qb)===version(hit.qbVersion)?'EXACT':'EQUIVALENT';current=resolution(hit,qb,mode);}else current=inherited(qb,api)||fallback(qb,api);try{global.dispatchEvent(new CustomEvent('weigg:release-profile',{detail:{profile:current,certified:isCertified(),resolutionMode:current&&current.resolutionMode||'FALLBACK'}}));}catch(_e){}return current;}
