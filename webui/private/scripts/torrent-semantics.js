(function(global){
  ...
  function progressPercent(value){var n=Number(value);if(!Number.isFinite(n)||n<0)n=0;if(n<=1)n*=100;return Math.max(0,Math.min(100,Math.round(n*100)/100));}
  function progressVisual(torrent){var t=torrent||{},code=String(t.state||''),percent=progressPercent(t.progress),dl=Math.max(0,Number(t.dlspeed)||0),up=Math.max(0,Number(t.upspeed)||0);if(/^(?:error|missingFiles)$/i.test(code))return{state:'error',active:false,percent:percent};if(/^(?:paused|stopped)(?:DL|UP)?$/i.test(code))return{state:'paused',active:false,percent:percent};if(/^(?:allocating|checkingDL|checkingUP|checkingResumeData|moving)$/i.test(code))return{state:'checking',active:false,percent:percent};if(/^(?:queuedDL|queuedUP)$/i.test(code))return{state:'queued',active:false,percent:percent};if(/^(?:stalledUP|stalled_uploading)$/i.test(code))return{state:'seed-idle',active:false,percent:percent};if(/^(?:uploading|forcedUP)$/i.test(code))return{state:up>0?'seed':'seed-idle',active:up>0,percent:percent};if(/^(?:stalledDL|stalled_downloading)$/i.test(code))return{state:'download-idle',active:false,percent:percent};if(/^(?:downloading|forcedDL|metaDL|forcedMetaDL)$/i.test(code))return{state:dl>0?'download':'download-idle',active:dl>0,percent:percent};if(percent>=100)return{state:'complete',active:false,percent:percent};return{state:'idle',active:false,percent:percent};}
