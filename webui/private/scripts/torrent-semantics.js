(function(global){
  ...
  function progressPercent(value){var n=Number(value);if(!Number.isFinite(n)||n<0)n=0;var ratio=n<=1?n:n/100;ratio=Math.max(0,Math.min(1,ratio));var percent=ratio*100,C=registry(),identity=C&&C.releaseIdentity?C.releaseIdentity():null,version=identity&&identity.qbVersion?String(identity.qbVersion):'',modern=C&&C.compareVersions&&version?C.compareVersions(version,'5.2.0')>=0:false;if(modern)percent=Math.trunc((percent+Number.EPSILON)*10)/10;else{percent=Math.round((percent+Number.EPSILON)*10)/10;if(percent===100&&ratio!==1)percent=99.9;}return Math.max(0,Math.min(100,percent));}
  function progressText(value){return progressPercent(value).toFixed(1)+'%';}
  function presentation(torrent){var t=torrent||{},code=String(t.state||''),percent=progressPercent(t.progress),dl=Math.max(0,Number(t.dlspeed)||0),up=Math.max(0,Number(t.upspeed)||0);if(/^(?:error|missingFiles)$/i.test(code))return{state:'error',tone:'danger',active:false,percent:percent};if(/^(?:allocating|checkingDL|checkingUP|checkingResumeData|moving)$/i.test(code))return{state:'checking',tone:'checking',active:true,percent:percent};if(/^(?:queuedDL|queuedUP)$/i.test(code))return{state:'queued',tone:'queued',active:false,percent:percent};if(/^(?:paused|stopped)(?:DL|UP)?$/i.test(code))return{state:percent>=100?'complete-idle':'paused',tone:percent>=100?'stopped-complete':'stopped-incomplete',active:false,percent:percent};if(/^(?:stalledUP|stalled_uploading)$/i.test(code))return{state:'seed-idle',tone:'stalled-up',active:false,percent:percent};if(/^(?:uploading|forcedUP)$/i.test(code))return{state:up>0?'seed':'seed-idle',tone:'seed',active:up>0,percent:percent};if(/^(?:stalledDL|stalled_downloading)$/i.test(code))return{state:'download-idle',tone:'stalled-down',active:false,percent:percent};if(/^(?:downloading|forcedDL|metaDL|forcedMetaDL)$/i.test(code))return{state:dl>0?'download':'download-idle',tone:'download',active:dl>0,percent:percent};if(percent>=100)return{state:'complete-idle',tone:'stopped-complete',active:false,percent:percent};return{state:'idle',tone:'muted',active:false,percent:percent};}
  function progressVisual(torrent){var v=presentation(torrent);return{state:v.state,active:v.active,percent:v.percent};}
  function stateTone(torrent){return presentation(torrent).tone;}
