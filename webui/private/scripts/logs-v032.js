(function(global){
  ...
  var MAX_ITEMS=5000;
  function tr(key,vars,fallback){var I=W.V036I18n;return I&&I.t?I.t(key,vars):(fallback||key);}
  ...
  function buildShell(root){
    ...
    var head=document.createElement('div');head.className='logs-v032-head';head.innerHTML='<span>'+tr('v036.logs.log',null,'Log')+'</span><span>'+tr('v036.logs.time',null,'Time')+'</span><span>'+tr('v036.logs.level',null,'Level')+'</span>';
    var list=document.createElement('div');list.className='logs-v032-list';list.setAttribute('role','list');
    var empty=document.createElement('div');empty.className='logs-v032-empty is-hidden';empty.textContent=tr('v036.logs.empty',null,'No logs match the current filters.');
  ...
  function renderRows(reset,added){
    ...
    if(!state.virtual||state.virtual.el!==list){
      state.virtual=new W.VirtualList(list,{rowHeight:54,overscan:8,renderRow:function(x){
        var row=document.createElement('div');row.className='logs-v032-row';row.dataset.tone=typeTone(Number(x.type));row.setAttribute('role','listitem');row.dataset.logId=String(itemId(x));
        ...
        var level=document.createElement('span');level.className='logs-v032-level status-pill';level.dataset.tone=typeTone(Number(x.type));level.textContent=typeLabel(Number(x.type));
  ...
  function showUnsupported(){var root=U.$('logs-content');if(!root)return;state.rendering=true;root.innerHTML='<div class="logs-v032-message-state">'+tr('v036.logs.unsupported',null,'This qBittorrent instance does not expose the log API.')+'</div>';state.rendering=false;}
  function showError(e){var root=U.$('logs-content');if(!root)return;state.rendering=true;root.innerHTML='<div class="logs-v032-message-state">'+tr('v036.logs.failed',{error:String(e&&e.message||e)},'Failed to read logs: '+String(e&&e.message||e))+'</div>';state.rendering=false;}
